...
int do_sprite_test(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	__maybe_unused int ret;
	printf("sunxi work mode=0x%x\n", get_boot_work_mode());
	if (get_boot_work_mode() == WORK_MODE_USB_PRODUCT) {
#if CONFIG_IS_ENABLED(AW_EFEX)
#ifdef CONFIG_SUNXI_SPRITE_LED
		sprite_led_init();
#endif
		printf("run usb efex\n");
		sunxi_usb_main_loop(SUNXI_USB_DEVICE_EFEX, 2500);
#ifdef CONFIG_SUNXI_SPRITE_LED
		sprite_led_exit(ret);
#endif
#endif
	} else if (get_boot_work_mode() == WORK_MODE_CARD_PRODUCT) {
#if CONFIG_IS_ENABLED(SUNXI_CARD_PRODUCT)
...
//		dcache_disable();
		sunxi_usb_main_loop(SUNXI_USB_DEVICE_EFEX, 0);
#endif
	} else {
		printf("others\n");
	}
