...
int do_timer_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	int base_count = 1000;

	if (timer_test_flag[0]) {
		pr_err("can not test timer 0 now\n");
		return -1;
	}
...
U_BOOT_CMD(timer_test, 2, 0, do_timer_test, "do a timer and int test",
	   "[delay time]");
...
int do_timer_test1(struct cmd_tbl *cmdtp, int flag, int argc,
		   char *const argv[])
{
	int base_count = 1000;

	if (timer_test_flag[1]) {
		pr_err("can not test timer 1 now\n");
		return -1;
	}
...
U_BOOT_CMD(timer_test1, 2, 0, do_timer_test1, "do a timer and int test",
	   "[delay time]");
