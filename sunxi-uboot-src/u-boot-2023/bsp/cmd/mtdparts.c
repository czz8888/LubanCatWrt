...
struct mtd_device *current_mtd_dev;
u8 current_mtd_partnum;
...
static int _sunxi_do_mtdparts(struct cmd_tbl *cmdtp, int flag, int argc,
		       char * const argv[])
{
	int ret;
	...
	if (!(gd->flags & GD_FLG_ENV_READY)) {
		/* setting default env may change baudrate, we will fix it later */
