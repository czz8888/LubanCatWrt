...
static int do_memtester(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	ul loops, loop, i;
	...
	memsuffix = argv[1 + uboot_opt_offset][strlen(argv[1 + uboot_opt_offset])-1];
	//printf("%s %s %s,%d\n", argv[0], argv[1], argv[2],strlen(argv[1]));

	memset(szSize, 0, sizeof(szSize));
	if (memsuffix ==  '\0')
		memcpy(szSize, argv[1 + uboot_opt_offset], strlen(argv[1 + uboot_opt_offset]));
	else
		memcpy(szSize, argv[1 + uboot_opt_offset], strlen(argv[1 + uboot_opt_offset]) - 1);
	...
	loops = simple_strtoul(argv[2 + uboot_opt_offset], NULL, 10);
	...
	while (!done_mem) {
		if (5 == argc) {
			buf = (void volatile *)simple_strtoul(argv[2], NULL, 10);
		} else {
			while (!buf && wantbytes) {
	...
	if (!do_mlock)
		printf("Continuing with unlocked memory; testing "
