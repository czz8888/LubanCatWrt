 ...
 */
#if defined(CONFIG_CMD_SAVEENV) && defined(CONFIG_CMD_EDITENV)
		      ...
		      char *const argv[])
{
	...
	extern char *env_table[9];
	int i;

	for (i = 0; i < ARRAY_SIZE(env_table); i++) {
		if (memcmp(env_table[i], argv[1], strlen(env_table[i])) == 0) {
			printf("cat not setenv %s by cmdline\n", env_table[i]);
			return -1;
		}
	}
#endif
...
#ifndef CONFIG_SUNXI_BOOT_ENV_INLINE
