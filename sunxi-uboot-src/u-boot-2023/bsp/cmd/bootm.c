...
{
	ulong __maybe_unused os_load_addr = simple_strtoul(argv[1], NULL, 16);
	int states;
	int ret;
	...
	if (gd->securemode) {
		...
		if (sunxi_verity_hash_tree(env_get("root_partition"), "rootfs") != 0) {
			pr_err("sunxi verity rootfs error\n");
			return -1;
		}
		extern void update_dm_verity_bootargs(void);
		update_dm_verity_bootargs();
		...
		if (sunxi_verify_os(os_load_addr, "boot") != 0) {
			return -1;
		}
	}
