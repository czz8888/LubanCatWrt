...
int do_hash_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	u8 hash[32] = { 0 };
...
int do_sm2_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	...
	sm2_ctx.mode = 3;
	if (sm2_ctx.mode == 3) {
		pr_debug("sm2 signature verify start\n");
		ret1 = sunxi_sm2_sign_verify_test(&sm2_ctx);
		if (ret1 == 0) {
...
int do_rsa_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	/* 2048 bit */
	u8 rsa[256]  = { 0 };
...
int do_rng_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
...
int do_aes_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (!byte_algin) {
		if ((src_addr & (CACHE_LINE_SIZE - 1)) != 0) {
			pr_err("start addr 0x%lx not aligned with cache line size 0x%x\n",
		...
		if ((enc_addr & (CACHE_LINE_SIZE - 1)) != 0) {
			pr_err("enc addr 0x%lx not aligned with cache line size 0x%x\n",
					enc_addr, CACHE_LINE_SIZE);
			return CMD_RET_FAILURE;
		}

		if ((dec_addr & (CACHE_LINE_SIZE - 1)) != 0) {
			pr_err("dec addr 0x%lx not aligned with cache line size 0x%x\n",
					dec_addr, CACHE_LINE_SIZE);
			return CMD_RET_FAILURE;
		}
	}

	printf("aes test start 0\n");
	...
	if (ret != 0) {
		...
		printf("aes test sucess\n");
	}

	return CMD_RET_SUCCESS;
}

static struct cmd_tbl cmd_ce_test[] = {
...
int do_sunxi_ce_test(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	struct cmd_tbl *cp;
