...
extern int sunxi_riscv_init(ulong img_addr, ulong run_ddr, u32 riscv_id);
				...
				unsigned long addr, unsigned long size)
{
...
#if 0
	desc = blk_get_devnum_by_uclass_idname("sunxi_flash", 0);
	if (desc == NULL) {
	...
	pr_info("partinfo: name %s, start 0x%lx, size 0x%lx\n", info.name,
				...
				unsigned long addr, unsigned long size)
{
	...
	if (!shdr) {
		pr_err("Can't find .digest section from %s\r\n", partition);
		pr_err("%s partition data damaged\r\n", partition);
		return -ENODEV;
	}
	pr_info("find .digest section from %s\r\n", partition);

	img_size = *(uint32_t *)(addr + shdr->sh_offset);
	pr_info("the img_size is %u\n", img_size);
...
// used in sunxi_riscv_init
int rv_fw_partition_sectors;
int rv_fw_partitions_num;
const char **rv_fw_partitions;

/*******************************************************************/
/* bootrv - boot application image from image in memory */
/*******************************************************************/
int do_bootrv(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	pr_info("temporary mem: 0x%lx+0x%lx\r\n", (long)mem_addr, (long)mem_size);
#ifdef CONFIG_SUNXI_VERIFY_RV
	if (argc >= 4) {
		rv_fw_partitions_num = argc - 4;
		rv_fw_partitions = (const char **)&argv[4];
	}
	for (i = 4; i < argc; i++) {
		if (do_sunxi_flash_read(argv[i], mem_addr, mem_size) <= 0)
			return -EBUSY;

#ifdef CONFIG_SUNXI_VERIFY_RISCV
		if (gd->securemode) {
			if (!sunxi_verify_riscv(mem_addr, mem_size, riscv_id)) {
				break;
			}
		} else {
	...
	if (i == argc) {
		...
		pr_err("the argc is %d\n", argc);
		return -1;
	}
#else

	rv_fw_partitions = (const char **)&argv[4];
	rv_fw_partitions_num = 1;
	if (argc > 4) {
		if (do_sunxi_flash_read(argv[4], mem_addr, mem_size) <= 0)
			return -EBUSY;
#ifdef CONFIG_SUNXI_VERIFY_RISCV
		if (gd->securemode) {
			if (sunxi_verify_riscv(mem_addr, mem_size, riscv_id) < 0) {
				return -1;
			}
		}
#endif
...
#endif
	rv_fw_partition_sectors = (mem_size + 512 - 1) / 512;
	...
	if (i > 4) {
		...
		for (i = 4; i < argc; i++) {
			...
			pr_info("restore %s partition from %s partition\r\n",
							argv[i], argv[avail]);
...
#endif
#else
	pr_err("Don't Support Any Riscv PlatForm.\n");
#endif
	return  0;
}
...
static char bootrv_help_text[] =
	...
	"\t'arg[2]' can be the length of load firmware\n"
	"\t'arg[3]' can be cpu id of the ip\n"
	"\t'arg[4]' can be partition name of riscv firmware\n"
	"\t'arg[5]' can be backup partition name of riscv firmware\n";
#endif
