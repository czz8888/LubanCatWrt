...
static struct env_driver *env_driver_lookup(enum env_operation op, int prio)
{
	...
	if (!drv) {
		sunxi_debug(NULL, "%s: No environment driver for location %d\n", __func__,
		      loc);
		return NULL;
	}
...
int env_load(void)
{
	...
	for (prio = 0; (drv = env_driver_lookup(ENVOP_LOAD, prio)); prio++) {
		...
		sunxi_info(NULL, "Loading Environment from %s... ", drv->name);
		/*
	 ...
	 */
	if (best_prio >= 0)
		sunxi_debug(NULL, "Selecting environment with bad CRC\n");
	else
		best_prio = 0;
...
int env_reload(void)
{
	...
	if (drv) {
		...
		if (!env_has_inited(drv->location)) {
			sunxi_err(NULL, "not initialized\n");
			return -ENODEV;
		}
...
int env_save(void)
{
	...
	if (drv) {
		...
		sunxi_info(NULL, "Saving Environment to %s... ", drv->name);
		if (!drv->save) {
			sunxi_err(NULL, "not possible\n");
			return -ENODEV;
		}
...
int env_erase(void)
{
	...
	if (drv) {
		...
		sunxi_info(NULL, "Erasing Environment on %s... ", drv->name);
		ret = drv->erase();
		if (ret)
			sunxi_info(NULL, "Failed (%d)\n", ret);
		else
			sunxi_info(NULL, "OK\n");
...
int env_init(void)
{
	...
	for (prio = 0; (drv = env_driver_lookup(ENVOP_INIT, prio)); prio++) {
		...
		sunxi_debug(NULL, "%s: Environment %s init done (ret=%d)\n", __func__,
		      drv->name, ret);
...
int env_select(const char *name)
{
	...
	sunxi_info(NULL, "Select Environment on %s: ", name);
	...
	if (!found) {
		sunxi_err(NULL, "driver not found\n");
		return -ENODEV;
	}
	...
	sunxi_warn(NULL, "priority not found\n");
