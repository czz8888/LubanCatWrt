...
#include <smc.h>
#include "private_storage_crypt.h"
...
{
	int id;
	int item_id = __write_protect_item_id(name);
			     ...
			     int buffer_len, int *data_len)
{
	...
	ret = sunxi_private_storage_read(item_name, secure_object, 4096,
					&retLen);
	if (ret) {
		return -1;
	}
...
int sunxi_secure_object_write(const char *item_name, char *buffer, int length)
{
	...
	return sunxi_private_storage_write(item_name, (char *)so, sizeof(*so));
}
...
static int secure_object_op_test(void)
{
	...
	tagt = (unsigned char *)memalign(CONFIG_SYS_CACHELINE_SIZE,
	...
	ret = sunxi_private_storage_write("HDMI", (void *)tagt, retLen);
	if (ret < 0) {
	...
	ret = sunxi_private_storage_read("HDMI", (void *)sec_buf, 4096,
					(int *)&retLen);
	if (ret < 0) {
	...
	ret = sunxi_private_storage_write("Widevine", (void *)tagt, retLen);
	if (ret < 0) {
	...
	ret = sunxi_private_storage_read("Widevine", (void *)sec_buf, 4096,
					(int *)&retLen);
	if (ret < 0) {
...
extern int sunxi_flash_pristorage_read(int item, unsigned char *buf,
				       unsigned int len);
extern int sunxi_flash_pristorage_write(int item, unsigned char *buf,
					unsigned int len);
...
int clear_secure_store(int index)
{
	...
	if (index == 0xffff) {
		...
		for (; i < 32; i++) {
			printf("..");
			sunxi_flash_pristorage_write(i, (void *)_inner_buffer,
						     4096);
		...
		printf("..");
		sunxi_flash_pristorage_write(index, (void *)_inner_buffer,
					     4096);
		printf("clearn %d done\n", index);
	}
	return 0;
...
static int sunxi_secure_object_list(void)
{
	...
	if (sunxi_flash_pristorage_read(0, _inner_buffer, 4096) < 0) {
	...
	while (*buf_start != '\0') {
		...
		if (!strncmp("key_burned_flag", name,
			...
			ret = sunxi_private_storage_read(name, (void *)buffer,
							4096, &retLen);
		else
...
{
	sunxi_private_storage_info_t *secdata =
		(sunxi_private_storage_info_t *)secdata_buf;
	char lower_name[64];
	int i = 0, ret = 0, align_len = 0;
	...
	if (gd->securemode == SUNXI_SECURE_MODE_WITH_SECUREOS) {
		if (!strncmp("hdcpkeyV22", secdata->name,
			     strlen("hdcpkeyV22")) ||
		    !strncmp("hdcpkey", secdata->name, strlen("hdcpkey"))) {
