...
static int sunxi_verify_embed_signature(void *buff, unsigned int len,
					...
					unsigned cert_len);
#ifndef CONFIG_SUNXI_VERIFY_BOOT_EMBED_SIGN
...
static int android_fitimage_get_signature(const struct fdt_header *hdr,
				       ulong *sign_data, ulong *sign_len);
...
//	struct blk_desc *desc;
	...
	int ret, aw_cert_exit = 0;
	struct andr_img_hdr *fb_hdr = (struct andr_img_hdr *)os_load_addr;
	const char *part_name = env_get("boot_partition");

#if CONFIG_IS_ENABLED(FIT)
	struct fdt_header *fdt_hdr;
	fdt_hdr = (struct fdt_header *)os_load_addr;
#endif
#if 0

	desc = blk_get_devnum_by_uclass_idname("sunxiflash", 0);
	if (desc == NULL)
		return -ENODEV;
	...
	ret = sunxi_partition_get_info(part_name, &info);
	if (ret < 0)
		return -ENODEV;
#endif
	...
	if (!memcmp(fb_hdr->magic, ANDR_BOOT_MAGIC, 8)) {
		...
		aw_cert_exit = android_image_get_signature(fb_hdr, &sign_data, &sign_len);
	}
#if CONFIG_IS_ENABLED(FIT)
	else if (fdt_magic(fdt_hdr) == FDT_MAGIC) {
		total_len = be32_to_cpu(fdt_hdr->totalsize);
		aw_cert_exit = android_fitimage_get_signature(fdt_hdr, &sign_data, &sign_len);
	}
#endif
	else {
		printf("error img format\n");
		return -1;
	}

	pr_notice("kernel len:%ld, part len:%ld\n", total_len, info.size * 512);
...
static int android_fitimage_get_signature(const struct fdt_header *hdr,
				       ulong *sign_data, ulong *sign_len)
{
	struct boot_img_fdt_ex *hdr_ex;
	ulong addr = 0;
	u32 total_len;

	hdr_ex = (struct boot_img_fdt_ex *)hdr;
	if (strncmp((void *)(hdr_ex->cert_magic), AW_CERT_MAGIC,
		    strlen(AW_CERT_MAGIC))) {
		printf("No cert image embeded, image %s\n", hdr_ex->cert_magic);
		return 0;
	...
	total_len = ALIGN(be32_to_cpu(hdr->totalsize), AW_PAGESIZE) + AW_PAGESIZE;
	addr = (ulong)hdr;
	addr += total_len;
...
#else
	ret = sunxi_efuse_verify_rotpk_dm(key_hash);
#endif
	if (ret < 0) {
...
int sunxi_verify_rotpk_hash(void *input_hash_buf, int len)
{
	...
	pr_info("show hash of publickey in certif\n");
...
static int cal_partioin_len(struct disk_partition *info)
{
...
#define SQUASHFS_MAGIC 0x73717368
	...
	pr_info("squashfs len:%d, part len:%ld\n", len, info->size * 512);
...
#define SUNXI_X509_CERTIFF_MAX_LEN 4096
	...
	printf("verify squashfs hash_tree success\n");
			...
			const char *part_name, const char *cert_name, int full)
{
...
#if 0
	pr_info("pattern size:%d, interval:%d,cnt:%d, whole_sample_len:%d, "
	...
	#if 0
			pr_info("from %lx read %d block:to %p\n",
			   ...
			   size_t vb_len, const char *pubkey_in_toc1)
{
	...
	if (sunxi_avb_get_hash_descriptor_by_name(image_name, vb_data, vb_len,
						  &desc)) {
		pr_err("get descriptor for %s failed\n", image_name);
		if (strcmp(image_name, pubkey_in_toc1) != 0) {
			...
			if (ret == 0) {
				...
				if (ret == 0) {
					pr_info("verify passed with non vbmeta partition signature\n");
				}
			} else {
				pr_err("read vbmeta in %s partition failed\n",
					 image_name);
			}
			if (vb_meta_data)
				free(vb_meta_data);
	...
	if (ret) {
		if (ret == -2) {
			...
			pr_err("vbmeta self verify failed\n");
	...
	if (ret == -2) {
		...
		if (check_public_in_rootcert(slot_vbmeta, &sub_certif)) {
			pr_err("self sign key verify failed\n");
			goto descriptot_need_free;
		}
	}
	...
	if (image_len != hdh->image_size) {
		pr_err("image_len not match, actual:%d, expected:%lld\n",
...
#else
	sunxi_sha_calc_dm(hash_result, 32, (uint8_t *)image_data - hdh->salt_len,
		       hdh->salt_len + image_len);
#endif
	...
	if (memcmp(expected_hash, hash_result, 32) != 0) {
		...
		if (ret) {
			pr_err("hash not match, hash of file:\n");
			sunxi_dump(hash_result, 32);
			pr_err("hash in descriptor:\n");
...
int sunxi_verify_dsp(ulong img_addr, u32 image_len, u32 dsp_id)
{
	...
	if (gd->securemode) {
		...
		if (ret == 0) {
			pr_err("find sunxi_image_header\n");
			if (sunxi_image_verify(img_addr, cert_name) != 0) {
				pr_err("dsp %s verify failed\n", cert_name);
				return -1;
			} else {
				pr_info("dsp %d verify success!\n", dsp_id);
			}
		} else {
			pr_err("not find sunxi_image_header\n");
			return -2;
		}
#else
		if (!image_len) {
			pr_err("dsp len is zero\n");
			return -1;
		}
		ret = sunxi_verify_signature((void *)img_addr, image_len, cert_name);
		if (ret < 0) {
			pr_err("dsp: sunxi_verify_signature fail: %d\n", ret);
			return -2;
		}
#endif
	}
...
int sunxi_verify_riscv(ulong img_addr, u32 image_len, u32 riscv_id)
{
	...
	if (gd->securemode) {
		...
		ret = sunxi_image_header_check((sunxi_image_header_t *)img_addr);
		if (ret == 0) {
			pr_info("find sunxi_image_header\n");
			if (sunxi_image_verify(img_addr, cert_name) != 0) {
				pr_err("riscv %s verify failed\n", cert_name);
				return -1;
			} else {
				pr_info("riscv %d verify success!\n", riscv_id);
			}
		} else {
			pr_err("not find sunxi_image_header\n");
			return -2;
...
#else
		image_len = sunxi_rproc_get_fw_size(img_addr);
		if (!image_len) {
			pr_err("riscv len is zero\n");
			return -1;
		}
		ret = sunxi_verify_signature((void *)img_addr, image_len, cert_name);
		if (ret < 0) {
			pr_err("riscv: sunxi_verify_signature fail: %d\n", ret);
			return -2;
		}
		pr_info("riscv %d verify success!\n", riscv_id);
#endif
			...
			uint8_t *key_e, uint32_t e_len, const char *name)
{
...
#else
	ret = sunxi_sha_calc_dm(key_hash, 32, align, 2048 / 8 * 2);
#endif
	if (ret) {
		pr_err("sunxi_sha_calc: calc rsa pubkey sha256 with hardware err\n");
		return ret;
	}
	...
	ret = smc_tee_check_hash(request_key_name, key_hash);
	if (ret == 0xFFFF000F) {
		pr_err("optee return pubkey hash invalid\n");
		return -1;
	} else if (ret == 0) {
		pr_info("pubkey %s valid\n", name);
	} else {
		pr_err("pubkey %s not found\n", name);
		return -1;
	}
...
#else
	ret = sunxi_sha_calc_dm(key_hash, 32, align, ecc_pkey_len);
#endif
	if (ret) {
		pr_err("sunxi_sha_calc: calc ecc pubkey sha256 with hardware err\n");
		return ret;
	}
...
int sunxi_image_verify(ulong os_load_addr, const char *cert_name)
{
	...
	if (tlv->th_pkey_type == PKEY_TYPE_RSA) {
		...
		pr_err("not support the pkey type %d\n", tlv->th_pkey_type);
		return -1;
	}
	...
	if (ret) {
		pr_err("calc file sha256 with hardware err\n");
		return -1;
	}
	...
	if (tlv->th_pkey_type == PKEY_TYPE_RSA) {
		ret = sunxi_rsa_calc(key_n, 256,
				...
				sign, 256);
		if (ret) {
			pr_err("sunxi_rsa_calc: calc rsa2048 with hardware err\n");
			return -2;
		}
		...
		if (!uECC_verify(ecc_pkey, hash_of_file,
			...
			pr_err("uECC_vierfy error\n");
			return -3;
		}
#else
		pr_err("can not find ecc api\n");
		return -3;
#endif
	}
