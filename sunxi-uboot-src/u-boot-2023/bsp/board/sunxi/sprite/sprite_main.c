...
int sunxi_card_sprite_main(int workmode, char *name)
{
	...
	if (sunxi_sprite_deal_private_ubidev(production_media)) {
		printf("sunxi sprite error : download private ubi area error\n");
		return -1;
	}
#endif
#ifdef CONFIG_SUNXI_DUAL_STORAGE
	production_media = sunxi_fdt_get_boot_storage_type(working_fdt);
#endif
	...
	if (sprite_next_work == SUNXI_UPDATA_NEXT_ACTION_SPRITE_TEST) {
		...
		if (!sunxi_flash_phyread(CONFIG_SUNXI_SDMMC_BOOT0_START_ADDR, 1024 / 512,
					 boot0_head_buf)) {
		...
		if (!sunxi_flash_phywrite(CONFIG_SUNXI_SDMMC_BOOT0_START_ADDR, 1024 / 512,
					  boot0_head_buf)) {
