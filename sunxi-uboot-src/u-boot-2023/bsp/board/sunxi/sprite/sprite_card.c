...
{
	/* thisfile thisfile is the partition name
	 * hard-coded by the card making tool.
	 */
	uint ret = sunxi_partition_get_offset_byname("thisfile");
	if (ret > 0)
		return ret;
	else
...
int sunxi_sprite_deal_private_ubidev(int production_media)
{
	char *buffer;
	uint item_original_size;
	imgitemhd = Img_OpenItem(imghd, "12345678", "PRIVATE-UBIDEV-0");
	if (!imgitemhd) {
		printf("sprite update error: fail to open private item\n");
		return -1;
	}
	...
	if (!item_original_size) {
		printf("sprite update error: fail to get private item size\n");
		return -1;
	}

	buffer = (char *)memalign(CONFIG_SYS_CACHELINE_SIZE, ALIGN(item_original_size, CONFIG_SYS_CACHELINE_SIZE));

	/*获取uboot的数据*/
	if (!Img_ReadItem(imghd, imgitemhd, (void *)buffer, ALIGN(item_original_size, CONFIG_SYS_CACHELINE_SIZE))) {
		printf("update error: fail to read data from for private area\n");
		return -1;
	}
	Img_CloseItem(imghd, imgitemhd);
	imgitemhd = NULL;

	if (sunxi_sprite_download_private_ubidev(buffer, item_original_size, production_media)) {
		printf("update error: fail to write private area\n");
		return -1;
	}
	printf("sunxi_sprite_deal_private_area ok\n");
	free(buffer);
	return 0;
}
#endif
