...
int sunxi_sprite_erase_flash(void *img_mbr_buffer)
{
	...
	if (need_erase_flag != 0x12) {
		ret = sunxi_private_storage_write_or_read("set-active-boot-slot",
							 &boot_slot, 1, 1);
		if (ret >= 0) {
			boot_slot = 0x31;
			sunxi_private_storage_write_or_read(
...
#if CONFIG_SUNXI_PRIVATE_STORAGE
		sunxi_private_storage_erase_all();
#endif
		return 0;
	}
...
//		get_boot_storage_type() == STORAGE_NAND ? sunxi_sprite_exit(1) : 0;
	...
	if (sunxi_private_storage_init())
		return -1;
	if (sunxi_private_storage_erase_all() == -1)
		return -1;
#endif
