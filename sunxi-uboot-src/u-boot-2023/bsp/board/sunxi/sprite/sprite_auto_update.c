...
static int usb_img_already_burn;
...
static int auto_update_fetch_version(void *img_version)
{
	imgitemhd = Img_OpenItem(imghd, "12345678", "123456789VERSION");
	if (!imgitemhd) {
		printf("Img_OpenItem error 0\n");
		return 0;
	}

	if (!Img_Fat_ReadItem(imghd, imgitemhd, imgname, (void *)img_version,
				1024)) {
		printf("sunxi sprite error : read string of date failed\n");
		return 0;
	}
	Img_CloseItem(imghd, imgitemhd);
	imgitemhd = NULL;
	...
	if (sunxi_private_storage_init()) {
		...
		memset(date, 0, 128);
		sunxi_secure_object_read("firmware_version", date, sizeof(date),
				&retlen);
		printf("img_version %s, date %s\n", (char *)img_version, date);
		if (!strcmp((char *)img_version, (char *)date)) {
			printf("the firmware already burn to flash\n");
			return -1;
		}
...
static int auto_update_write_version(void *img_version)
{
	imgitemhd = Img_OpenItem(imghd, "12345678", "123456789VERSION");
	if (!imgitemhd) {
		printf("Img_OpenItem error 1\n");
		return -1;
	}
	...
	if (sunxi_private_storage_init()) {
		...
		if (sunxi_secure_object_write("firmware_version", (char *)img_version,
					strlen(img_version))) {
...
int sunxi_auto_update_main(void)
{
	...
	img_version = memalign(CONFIG_SYS_CACHELINE_SIZE, 1024);
	if (auto_update_fetch_version(img_version)) {
		usb_img_already_burn = 1;
		printf("sunxi sprite error : fetch date error\n");
		return -1;
	}
	...
	if (auto_update_write_version(img_version)) {
		printf("sunxi sprite error : write image version error\n");
	}

#ifdef CONFIG_SUNXI_SPRITE_CARTOON
	sprite_cartoon_upgrade(100);
#ifdef CONFIG_SUNXI_SPRITE_CARTOON_CHAR
	sprite_uichar_printf("Burn OK\n");
#endif
#endif
...
#ifdef CONFIG_SUNXI_AUTO_UPDATE_CHECK
int sunxi_auto_update_gpadc_key_check(int nodeoffset)
{
#ifdef CONFIG_SUNXI_GPADC
	int ret = 0;
	int val, val_max, val_min;
	uint gpadc_channel, target_vol, deviation, delay_ms;
	uint check = 0;

	ret = fdt_getprop_u32(working_fdt, nodeoffset, "gpadc_channel",
			(uint32_t *)&gpadc_channel);
	if (ret < 0) {
		printf("FDT ERROR:%s:get property gpadc_channel error\n", __func__);
		return -1;
	}
	ret = fdt_getprop_u32(working_fdt, nodeoffset, "target_vol",
			(uint32_t *)&target_vol);
	if (ret < 0) {
		printf("FDT ERROR:%s:get property target_vol error\n", __func__);
		return -1;
	}
	ret = fdt_getprop_u32(working_fdt, nodeoffset, "deviation",
			(uint32_t *)&deviation);
	if (ret < 0) {
		printf("FDT ERROR:%s:get property deviation error\n", __func__);
		return -1;
	}
	ret = fdt_getprop_u32(working_fdt, nodeoffset, "delay_ms",
			(uint32_t *)&delay_ms);
	if (ret < 0) {
		printf("FDT ERROR:%s:get property delay_ms error\n", __func__);
		return -1;
	}

	val_min = target_vol - deviation;
	val_max = target_vol + deviation;
	sunxi_gpadc_init();
	do {
		val = sunxi_get_gpadc_vol(gpadc_channel);
		pr_debug("gpadc read vol: %d \n", val);
		if (val >= val_min && val <= val_max) {
			check++;
		} else {
			printf("auto update key not press\n");
			return -1;
		}
		udelay(delay_ms * 1000);
	} while (check < 2);
	printf("auto update key press\n");
#endif
...
int do_auto_update_check(struct cmd_tbl *cmdtp, int flag, int argc,
			 char *const argv[])
{
	int arg_max = 0, i;
	...
	uintptr_t tmp_addr = (uintptr_t)simple_strtoul(env_boot_normal[2], NULL, 16);
	file_buff = (char *)tmp_addr;
	file_p = file_buff;
	u32 update_next_work;
	int nodeoffset = fdt_path_offset(working_fdt, FDT_PATH_AUTO_UPDATE);

	printf("begin auto update check\n");
#ifdef CONFIG_SUNXI_AUTO_UPDATE_CHECK
	u32 update_selet_mode;
	if (nodeoffset > 0) {
		if (fdt_getprop_u32(working_fdt, nodeoffset, "select_mode",
			(uint32_t *)&update_selet_mode) < 0) {
			update_selet_mode = SUNXI_AUTO_UPDATE_ALWAYS;
		}
		pr_info("update_select_mode=%d\n", update_selet_mode);
		if (update_selet_mode == SUNXI_AUTO_UPDATE_SKIP) {
			goto out;
		} else if (update_selet_mode == SUNXI_AUTO_UPDATE_GPADC_KEY) {
			ret = sunxi_auto_update_gpadc_key_check(nodeoffset);
			if (ret == -1)
				goto out;
		}
	}
...
// #endif
	if (!run_command("usb reset", 0)) {
		...
		printf("sunxi update check fail\n");
		return 0;
	}
	...
	if (simple_strtoul(argv[1], NULL, 16) == 1) {
		...
		file_size = 0;
		file_offset = 0;
		ret = fat_fs_read("update/auto_update.txt", (void *)file_p,
...
next_work:
	if (!usb_img_already_burn) {
		if (fdt_getprop_u32(working_fdt, nodeoffset, "next_work",
			(uint32_t *)&update_next_work) < 0) {
			update_next_work = SUNXI_UPDATE_NEXT_ACTION_SHUTDOWN;
		}
		printf("update_next_work=%d\n", update_next_work);
		sunxi_update_subsequent_processing(update_next_work);
	}
...
U_BOOT_CMD(auto_update_check, CONFIG_SYS_MAXARGS, 1, do_auto_update_check,
	"auto_update_v2   - Do TFCard or Udisk update\n",
	"\nnote:\n"
	"	- The auto_update.txt configuration file must be\n"
	"	added to the update/ directory of the Udisk.\n"
	...
	"		sunxi_flash write update/<imgname.img> firmware\n");
