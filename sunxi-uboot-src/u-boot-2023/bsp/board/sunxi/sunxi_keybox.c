...
static int sunxi_keybox_init_key_list_from_env(void)
{
	...
	if (!command_p) {
		pr_info("keybox_list not set in env, leave empty\n");
		key_list_inited = 1;
		return 0;
	}
...
static int try_reencrypt_and_install(const char *name, int replace)
{
	...
	if (ret) {
		...
		if (replace) {
			pr_info("re_encrypt works, replace current data with reencrypted data\n");
			ret = sunxi_secure_object_write(
				secdata.name, (void *)&secdata,
				SUNXI_SECURE_STORTAGE_INFO_HEAD_LEN +
					secdata.len);
			if (ret)
				pr_info("replace failed\n");
		}
		return 0;
	}
}
...
static int default_keybox_installation(const char *name)
{
	...
	if (ret) {
		...
		if (strcmp(name, secure_object.name) != 0) {
			pr_info("try reencrypt and install key %s\n", name);
...
static int sunxi_keybox_install_keys(void)
{
	...
	for (i = 0; i < key_list_cnt; i++) {
		...
		if ((ret >= 0) && (start[ret].key_load_cb != NULL)) {
			pr_info("load key %s with regesited cb\n", key_list[i]);
			start[ret].key_load_cb(key_list[i]);
			continue;
		}

		pr_info("load key %s with default cb\n", key_list[i]);
		/* deafult behavior */
...
//		return 0;
	...
	if (sunxi_probe_secure_os() == 0) {
		pr_info("no secure os for keybox operation\n");
		return 0;
	}
			  ...
			  int write_protect)
{
	...
	if ((i >= 0) && (start[i].key_burn_cb != NULL)) {
		pr_info("burning key %s with regesited cb\n", name);
		return start[i].key_burn_cb(name, buf, key_len, encrypt,
					    write_protect);
	}
	/* default behavior */
	pr_info("burning key %s with default cb\n", name);
...
int android_trust_chain_load_cb(const char *name)
{
	...
	pr_info("trust chain key, load in fix order,"
...
int android_drmkey_load_cb(const char *name)
{
	if (default_keybox_installation(name) != 0) {
		...
		pr_info("set android_drmkey to true\n");
...
int sunxi_mmc_rpmb_burn_key(void *key)
{
	...
	/* when emmc boot, curr_device will be 0 always */
	curr_device = 0;
#define RPMB_SZ_DATA 256
...
#endif
	if (blk_select_hwpart_devnum(UCLASS_MMC, curr_device, MMC_PART_RPMB) != 0)
		return -1;
	...
	blk_select_hwpart_devnum(UCLASS_MMC, curr_device, original_part);
	return ret;
}
		  ...
		  __maybe_unused int encrypt, __maybe_unused int write_protect)
{
	...
	ret = sunxi_mmc_rpmb_burn_key((void *)buf);
	if (ret == -3) {
		pr_info("rpmb key burned, key valid, only store key\n");
	} else if (ret == -2) {
