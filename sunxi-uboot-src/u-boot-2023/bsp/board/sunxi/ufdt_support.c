...
void get_andriod_dtbo_idx(const char *cmdline, const char *name)
{
	...
	if (cmdline == NULL) {
		pr_info("%s cmdline is NULL", __func__);
		return;
	}
	...
	pr_info("%s  can't find", name);
	return;
}

__maybe_unused void set_andriod_dtbo_idx(void)
{
	char *dtboidx = env_get("dtbo_idx");
	char dtbo_name_and_idx[32];

	sprintf(dtbo_name_and_idx, "  androidboot.dtbo_idx=%s", dtboidx);
	pr_info("dtbo_name_and_idx:%s\n", dtbo_name_and_idx);

	get_andriod_dtbo_idx(dtbo_name_and_idx, ANDR_BOOT_DTBO_MAIGC);
}

#define DTBOIMG_BUF_SIZE	(1024 * 1024 * 2)
#define DTO_PARTION			("dtbo")
#define DTBO_COUNT			(10)
extern int do_sunxi_flash(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[]);
...
int load_dtboimg(void *dtboimg_buf)
{
	...
	argv[1] = "read";
    argv[2] = dtboimg_head;
#ifndef CONFIG_SUNXI_SWITCH_SYSTEM
	argv[3] = DTO_PARTION;
#else
	argv[3] = env_get("dtbo_part_name");
#endif
	argv[4] = NULL;

	dtbo_partiton = env_get("dtbo_part_name");
	if (dtbo_partiton && dtbo_partiton[0] != '\0') {
		argv[3] = dtbo_partiton;
		pr_info("dtbo_part_name = %s\n", dtbo_partiton);
	}
