...
int reserve_sunxi_boot_param(void)
{
	...
	if (sunxi_verify_checksum(sunxi_boot_param,
		...
		pr_err("bootparam checksum not match\n");
		return 0;
	}
	if (sunxi_bootparam_check_magic(sunxi_boot_param) < 0) {
		pr_err("bootparam magic error\n");
		return 0;
	}
	gd->start_addr_sp -= sunxi_boot_param_size;
	gd->sunxi_boot_param_addr = map_sysmem(gd->start_addr_sp, sunxi_boot_param_size);
	memcpy(gd->sunxi_boot_param_addr, (void *)sunxi_boot_param, sunxi_boot_param_size);
...
typedef_sunxi_boot_param *sunxi_bootparam_get_buf_in_relocate_f(void)
{
	if (uboot_spare_head.boot_data.work_mode == WORK_MODE_USB_PRODUCT)
		...
		return (typedef_sunxi_boot_param *)(CONFIG_TEXT_BASE +
