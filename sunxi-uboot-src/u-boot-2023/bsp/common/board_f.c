...
static int setup_reloc(void)
{
	...
	if (gd->flags & GD_FLG_SKIP_RELOC) {
		...
		pr_notice("Relocation Offset is: %08lx\n", gd->reloc_off);
...
#if defined(CONFIG_COMPONENTS_CERTIFICATION_60730)
extern int aw_startup_check_entry(void);
static int aw_iec_startup_check(void)
{
	if (get_boot_work_mode() == WORK_MODE_BOOT) {
		return aw_startup_check_entry();
	}
