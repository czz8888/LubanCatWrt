...
static int mmc_select_mode_and_width(struct mmc *mmc, uint card_caps)
{
	...
	if (mmc->selected_mode == MMC_HS_200 ||
		...
		/* In case the host send CMD to device when down to HS, switch to 50M first */
		mmc_set_clock(mmc, 50000000, MMC_CLK_ENABLE);
		mmc_mmc_switch_bus_mode(mmc, HSSDR52_SDR25, mmc->bus_width);
	} else
#endif
