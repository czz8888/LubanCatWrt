...
/* max host num*/
#define TUNING_MAX_HOST_NO 5

int sunxi_mmc_cfg_speed_mode(struct mmc *mmc)
{
...
int sunxi_flashmap_offset(enum flash_type flash_name, enum region_flash_type region_name)
{
	...
	case MMC_TUNING:
		...
		break;
	default:
		pr_err("%s-error map offset--%d\n", __func__, region_name);
	}

	return start;
}

ulong sunxi_mmc_bread(struct mmc *mmc, lbaint_t start, lbaint_t blkcnt, void *dst)
{
...
ulong sunxi_mmc_bwrite(struct mmc *mmc, lbaint_t start, lbaint_t blkcnt,
		 const void *src)
{
	struct blk_desc *block_dev = mmc_get_blk_desc(mmc);
...
int sunxi_need_rty(struct mmc *mmc)
{
	...
	err_no = sunxi_get_detail_errno(mmc);
	ret = sunxi_decide_retry(mmc, err_no, 0);
	if (!ret) {
...
int write_tuning_try_freq(struct mmc *mmc, u32 clk)
{
	...
	} while (!sunxi_decide_retry(mmc, err_no, 0));
...
static ulong sunxi_read_tuning(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
	...
	do {
		...
		if (sunxi_mmc_bread(mmc, start, cur, dst) != cur) {
...
{
	struct mmc *mmc = NULL;
	struct sunxi_mmc_priv *priv = NULL;
	...
	for (i = 0; i < TUNING_MAX_HOST_NO; i++) {
		mmc = find_mmc_device(i);
		if (mmc == NULL)
			continue;
		priv = (struct sunxi_mmc_priv *)mmc->priv;
		if (priv->mmc_no == dev_num) {
			MMCDBG("find the mmc = %d, from dev_num = %d\n", mmc->cfg->host_no, dev_num);
			break;
		}
	}

	if (mmc == NULL || i == TUNING_MAX_HOST_NO) {
...
mmc_read_retry:
	...
	ret = sunxi_mmc_bread(mmc, sunxi_flashmap_offset(FLASHMAP_SDMMC, BOOT_PARAM),
	 ...
	 if (pregion->header.magic == SDMMC_PARAMETER_MAGIC) {
		...
		printf("%s %d:region magic is not right, %s %x\n", __func__, __LINE__,
		 ...
		 if (retry_read < 3) {
				 ...
				 mmc_dumphex32("info", (char *)pregion_r, 16);
				 goto error;
		 }
	 }
...
retry:
		...
		ret = sunxi_mmc_bwrite(mmc, sunxi_flashmap_offset(FLASHMAP_SDMMC, BOOT_PARAM),
