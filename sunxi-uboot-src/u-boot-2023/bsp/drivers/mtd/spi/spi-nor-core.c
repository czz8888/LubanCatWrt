...
bool speed_lock = true;
static struct spi_nor *g_spinor;

#define CONFIG_SPINOR_DEFAULT_SPEED	(30000000)
unsigned int sample_delay = 0xaaaaffff;
unsigned int sample_mode = 0xaaaaffff;
				 ...
				 u_char *buf)
{
	...
	if (spi_nor_protocol_is_dtr(nor->read_proto))
		op.dummy.nbytes *= 2;
	*/
	/* op.dummy.nbytes represents cycles */
...
{
	struct spi_slave *spi = nor->spi;
	struct sunxi_spif *sspi = dev_get_priv(spi->dev->parent);
...
int spinor_update_right_delay_para(struct mtd_info *mtd)
{
	struct spi_nor *nor = mtd->priv;
	struct spi_slave *slave = nor->spi;
	uint32_t erasesize = mtd->erasesize;
	uint32_t len = nor->page_size;
	...
	sprintf(spi_board, "spi%d/spi-nor", CONFIG_SF_DEFAULT_BUS);
	fdt_off = fdt_path_offset(working_fdt, spi_board);
	if (fdt_off < 0) {
		pr_err("get spi-nor node from fdt failed\n");
		return -EINVAL;
	}
	val = dev_read_u32_default(slave->dev, "spi-max-frequency", CONFIG_SPINOR_DEFAULT_SPEED);
	if (val <= 0) {
		pr_err("get spi-max-frequency from node of spi-nor failed\n");
		return -EINVAL;
	}
	slave->max_hz = val;
	pr_info("set spi%d clk to %d MHz\n", CONFIG_SF_DEFAULT_BUS, val/1000/1000);
	...
	if (!len_ok) {
		...
		pr_err("spi nor update delay param error, use auto mode\n");
	} else {
		right_sample_delay = (start_ok + end_ok) / 2;
		sample_delay = right_sample_delay;
		sample_mode = mode_ok;
		sunxi_spi_calibrate_set_sample_param(mtd->dev, SUNXI_SPI_SAMP_MODE_MANUAL,
							sample_mode, sample_delay);
	}
	pr_err("spinor try spi Sample mode:%d right_delay:0x%x\n", sample_mode, sample_delay);
...
int spinor_set_right_delay_para(struct mtd_info *mtd)
{
	struct spi_nor *nor = mtd->priv;
	struct spi_slave *slave = nor->spi;
	u_char *buffer;
	...
	ret = mtd->_read(mtd, 0, 2048, &retlen, buffer);
	if (gd->securemode == 0) {
	...
	pr_info("spinor sample_mode:%x sample_delay:0x%x\n",
	...
	if ((boot_info->sample_delay || boot_info->sample_mode) &&
		...
		sunxi_spi_calibrate_set_sample_param(mtd->dev, SUNXI_SPI_SAMP_MODE_MANUAL,
				boot_info->sample_mode, boot_info->sample_delay);
		sample_delay = boot_info->sample_delay;
		sample_mode = boot_info->sample_mode;
		slave->max_hz = (boot_info->frequency) * 1000 * 1000;
	} else
		slave->max_hz = 24 * 1000 * 1000;
...
#elif CONFIG_SPI_SAMP_DL_EN
	if (get_boot_work_mode() == WORK_MODE_USB_PRODUCT ||
			get_boot_work_mode() == WORK_MODE_CARD_PRODUCT)
		spinor_update_right_delay_para(mtd);
	else {
		spinor_set_right_delay_para(mtd);
	}
#endif
