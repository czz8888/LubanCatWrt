...
extern int fdt_getprop_string(const void *fdt, int nodeoffset,
			const char *name, char **val);

extern struct udevice *cur_sunxi_flash_blk;
...
inline int aw_spinand_mtd_write(unsigned int start, unsigned int sects, void *buf)
{
	...
	align_buf = memalign(64, info->block_size(chip));
	if (!align_buf) {
...
#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT) || IS_ENABLED(CONFIG_SUNXI_BOOT_PARAM)
u32 get_mtd_part_offset(void *name)
{
	struct aw_spinand *spinand = get_spinand();
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	...
	unsigned int i = 0, block_num, uboot_end;
	char fdt_node_name[40];
	char *part_name;
	int fdt_off;
	u32 offset = 0;
	...
	while (1) {
		snprintf(fdt_node_name, 40,
			"/soc/sunxi_flashmap/nand_map/partition%d", i);

		fdt_off = fdt_path_offset(working_fdt, fdt_node_name);
		if (fdt_off < 0) {
			printf("fdt_node_name:%s not find\n", fdt_node_name);
			return 0;
		}

		fdt_getprop_string(working_fdt, fdt_off, "part_name", &part_name);

		if (!strcmp(part_name, name))
			break;

		fdt_getprop_u32(working_fdt, fdt_off, "phy_block_num", &block_num);

		offset += (block_num * phy_blk_bytes);
		i++;
	}

	pr_debug("part name:%s  offset:0x%x \n", part_name, offset);
	return offset;
}
#endif

#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT)
u32 get_mtd_part_size(void *name)
{
	struct aw_spinand *spinand = get_spinand();
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	...
	while (1) {
		...
		if (!strcmp(part_name, name)) {
			fdt_getprop_u32(working_fdt, fdt_off, "phy_block_num", &block_num);
			return block_num * phy_blk_bytes;
		}
	}
}
#endif
...
int spinand_mtd_get_flash_info(void *data, unsigned int len)
{
	...
	max_hz = dev_read_u32_default(chip->dev, "spi-max-frequency", CONFIG_SPINAND_DEFAULT_SPEED);
	if (max_hz < 0) {
	...
	boot_info->logic_start_block =
		sunxi_get_mtdpart_offset(get_mtd_num_by_name("sys")) / phy_blk_size;;
	boot_info->nand_specialinfo_page = 0;
	boot_info->nand_specialinfo_offset = 0;
	boot_info->physic_block_reserved = 0;

	if (chip->rx_bit == 1) {
		boot_info->OperationOpt &= ~SPINAND_DUAL_READ;
		boot_info->OperationOpt &= ~SPINAND_QUAD_READ;
	} else if (chip->rx_bit == 2)
		boot_info->OperationOpt &= ~SPINAND_QUAD_READ;
...
#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT)
	boot_info->boot_offset = get_mtd_part_offset("boot");
	if (!boot_info->boot_offset) {
		boot_info->boot_offset = (uboot_end + AW_SPINAND_RESERVED_PHY_BLK_FOR_SECURE_STORAGE) * phy_blk_size;
		pr_err("get boot offset = 0, default:0x%x\n", boot_info->boot_offset);
	}

	boot_info->boot_size = get_mtd_part_size("boot");
	if (!boot_info->boot_size) {
#if IS_ENABLED(CONFIG_BIMAGE_SIZE)
		boot_info->boot_size = CONFIG_BIMAGE_SIZE * 1024 * 1024;
#else
		pr_err("boot size = 0, CONFIG_BIMAGE_SIZE not define\n");
#endif
	}
	pr_debug("boot offset:0x%x .... \n", boot_info->boot_offset);
#endif

#ifdef CONFIG_SUNXI_BOOT_PARAM
	boot_info->boot_param_offset = get_mtd_part_offset("boot_param");
	if (!boot_info->boot_param_offset) {
		boot_info->boot_param_offset = (uboot_end + AW_SPINAND_RESERVED_PHY_BLK_FOR_SECURE_STORAGE) * phy_blk_size;
		pr_err("get boot param offset = 0, default:0x%x\n", boot_info->boot_param_offset);
	}
#endif
	return 0;
}
...
void spinand_download_boot_param(void)
{
	...
	if (!boot_param_entry) {
		pr_err("boot param partition not found\n");
		boot_param_entry = boot_info->boot_param_offset;
	}

	pr_debug("write boot param offset:%x\n", boot_param_entry);
	aw_spinand_mtd_write(boot_param_entry, BOOT_PARAM_SIZE >> 9, boot_param);
...
int spinand_mtd_download_uboot(unsigned int len, void *buf)
{
	...
	if (end - start < 2 * blks_per_uboot) {
		pr_err("no enough space for at least two uboot\n");
...
int spinand_mtd_init(unsigned int busnum, unsigned int cs,
		struct udevice **devp)
{
	...
	if (ret || strcmp(slave->dev->name, "spi-nand@0"))
		return -1;

	*devp = slave->dev;
	return 0;
#else
	#error "spinand does not support non-DM mode"
#endif
}
...
unsigned spinand_mtd_size(void)
{
	...
	unsigned int aw_spinand_info_total_size(struct aw_spinand_chip *chip);
	return aw_spinand_info_total_size(chip);
}
