		...
		struct aw_spinand_chip_request *req)
{
	if (req && (req->databuf || req->oobbuf)) {
		...
		cache->page = req->page;
		cache->area_data = INVALID_CACHE_ALL_AREA;
		cache->area_oob = INVALID_CACHE_ALL_AREA;
		if (req->databuf)
			cache->area_data = VALID_CACHE_DATA;
		if (req->oobbuf)
			cache->area_oob = VALID_CACHE_OOB;
		...
		struct aw_spinand_chip_request *req)
{
	...
	if (req->block != cache->block || req->page != cache->page)
		return false;
	if (req->databuf && cache->area_data != VALID_CACHE_DATA)
		return false;
	if (req->oobbuf && cache->area_oob != VALID_CACHE_OOB)
		return false;
	return true;
}

static int aw_spinand_cahce_write_to_cache_do(struct aw_spinand_chip *chip,
		void *buf, unsigned int len, unsigned int column, struct aw_spinand_chip_request *req)
{
	int ret;
	...
	if (chip->tx_bit == SPI_NBITS_QUAD)
		op.cmd.opcode = column ? SPI_NAND_RANDOM_PP_X4 : SPI_NAND_PP_X4;
	else
		op.cmd.opcode = column ? SPI_NAND_RANDOM_PP : SPI_NAND_PP;

	if ((pinfo->OperationOpt & SPINAND_TWO_PLANE_SELECT) &&
			(req->block % 2 == 1))
		op.addr.val |= (SPI_SELECT_ODDNUM_BLACK << 8);
		...
		struct aw_spinand_chip_request *req)
{
	...
	return aw_spinand_cahce_write_to_cache_do(chip,
			...
			half_page_size + info->phy_oob_size(chip), column, req);
}
		...
		struct aw_spinand_chip_request *req)
{
	...
	struct aw_spinand_phy_info *pinfo = info->phy_info;
	struct aw_spinand_cache *cache = chip->cache;
	struct spi_mem_op op = SPINAND_PAGE_READ_FROM_CACHE_OP(true, 0, 1, NULL, 0);
