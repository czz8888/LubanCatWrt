...
static int aw_spinand_power_reset(struct aw_spinand_chip *chip)
{
	int ret;
	struct spi_mem_op op = SPI_MEM_OP(SPI_MEM_OP_CMD(0x66, 1),
					  SPI_MEM_OP_NO_ADDR,
					  SPI_MEM_OP_NO_DUMMY,
					  SPI_MEM_OP_NO_DATA);
		...
		int len, int dummy)
{
	...
	ret = aw_spinand_power_reset(chip);
	if (ret)
		pr_err("aw_spinand_chip_reset fail\n");

	ret = aw_spinand_chip_reset(chip);
	if (ret)
		return ret;

	op.cmd.opcode = SPI_NAND_RDID;
	op.cmd.nbytes = 1;
	op.cmd.buswidth = 1;

	/* In order to be compatible with materials such as GD5F1GQ4UBYIG,
	 * the read ID is cmd + addr (0), we default to replace the read ID
	 * dummy with the address data 0 */
	if (dummy) {
		op.addr.val = 0;
		op.addr.nbytes = 1;
		op.addr.buswidth = 1;
		...
		struct aw_spinand_chip_request *req, unsigned int addr)
{
	 ...
	 */
	if (addrtmp >= spinand_sys_part_offset()) {
