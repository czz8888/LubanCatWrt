...
struct aw_spinand_phy_info gigadevice[] = {
	...
	{
		.Model          = "GD5F2GQ5REYIG",
		.NandID         = {0xc8, 0x42, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
		.DieCntPerChip  = 1,
		.SectCntPerPage = 4,
		.PageCntPerBlk  = 64,
		.BlkCntPerDie   = 2048,
	...
	{
		.Model		= "GD5F1GM9UEYIG",
		.NandID		= {0xc8, 0x91, 0x01, 0xff, 0xff, 0xff, 0xff, 0xff},
		.DieCntPerChip  = 1,
		.SectCntPerPage = 4,
		.PageCntPerBlk  = 64,
		.BlkCntPerDie	= 1024,
		.OobSizePerPage = 64,
		.OperationOpt	= SPINAND_QUAD_READ | SPINAND_QUAD_PROGRAM |
			SPINAND_DUAL_READ | SPINAND_CONTINUOUS_READ,
		.MaxEraseTimes  = 50000,
		.EccFlag	= HAS_EXT_ECC_SE01,
		.EccType	= BIT4_LIMIT5_TO_7_ERR8_LIMIT_12,
		.EccProtectedType = SIZE16_OFF0_LEN16,
...
struct aw_spinand_phy_info xtx[] = {
	...
	},
	{
		/* XT26G02D */
		.Model		= "XT26G02DWSIG",
		.NandID		= {0x0B, 0x32, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
		.DieCntPerChip  = 1,
		.SectCntPerPage = 4,
		.PageCntPerBlk  = 64,
		.BlkCntPerDie	= 2048,
	...
	},
	{
		/* XT26G04D */
		.Model		= "XT26G04DWSIG",
		.NandID		= {0x0B, 0x33, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
		.DieCntPerChip  = 1,
		.SectCntPerPage = 8,
		.PageCntPerBlk  = 64,
		.BlkCntPerDie	= 2048,
...
struct aw_spinand_phy_info fm[] = {
	...
	{
		.Model		= "FM25S04BI3",
		.NandID		= {0xa1, 0xd6, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
		.DieCntPerChip  = 1,
		.SectCntPerPage = 4,
		.PageCntPerBlk  = 64,
		.BlkCntPerDie	= 2048,
...
struct aw_spinand_phy_info dosilicon[] = {
	...
	{
		.Model		= "DS35X2GBXXX",
		.NandID		= {0xe5, 0xb2, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
		//.NandID		= {0xe5, 0xf2, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},	//Modfiy by JS
		.DieCntPerChip  = 1,
		.SectCntPerPage = 4,
		.PageCntPerBlk  = 64,
		.BlkCntPerDie	= 2048,
...
#define BAD_BLK_MARK_POS4 "last_2_page"
	...
	info.Model = dev_read_string(chip->dev, "model");
	if (!info.Model) {
	...
	u32 id = 0xffffffff;
	ret = dev_read_u32(chip->dev, "id-0", &id);
	if (ret < 0) {
	...
	id = 0xffffffff;
	ret = dev_read_u32(chip->dev, "id-1", &id);
	if (ret < 0) {
	...
	ret = dev_read_u32(chip->dev, "die_cnt_per_chip", &(info.DieCntPerChip));
	if (ret < 0) {
		pr_err("get spi-nand DieCntPerChip from fdt fail\n");
		goto err;
	}

	ret = dev_read_u32(chip->dev, "blk_cnt_per_die", &(info.BlkCntPerDie));
	if (ret < 0) {
		pr_err("get spi-nand BlkCntPerDie from fdt fail\n");
		goto err;
	}

	ret = dev_read_u32(chip->dev, "page_cnt_per_blk", &(info.PageCntPerBlk));
	if (ret < 0) {
		pr_err("get spi-nand PageCntPerBlk from fdt fail\n");
		goto err;
	}

	ret = dev_read_u32(chip->dev, "sect_cnt_per_page", &(info.SectCntPerPage));
	if (ret < 0) {
		pr_err("get spi-nand SectCntPerPage from fdt fail\n");
		goto err;
	}

	ret = dev_read_u32(chip->dev, "oob_size_per_page", &(info.OobSizePerPage));
	if (ret < 0) {
		pr_err("get spi-nand OobSizePerPage from fdt fail\n");
		goto err;
	}

	bad_blk_mark_pos = dev_read_string(chip->dev, "bad_block_mark_pos");
	if (!bad_blk_mark_pos) {
		...
		if (!memcmp(bad_blk_mark_pos, BAD_BLK_MARK_POS1, strlen(BAD_BLK_MARK_POS1)))
			...
			// info.BadBlockFlag = BAD_BLK_FLAG_FRIST_1_PAGE; Modfiy by JS
	...
	ret = dev_read_u32_default(chip->dev, "max_erase_times", -1);
	if (ret < 0) {
	...
	ret = dev_read_u32(chip->dev, "ecc_type", &(info.EccType));
	if (ret < 0) {
		pr_err("get spi-nand EccFlag from fdt fail\n");
		goto err;
	}

	ret = dev_read_u32(chip->dev, "ecc_protected_type", &(info.EccProtectedType));
	if (ret < 0) {
		pr_err("get spi-nand ecc_protected_type from fdt fail\n");
		goto err;
	}

	ret = dev_read_u32(chip->dev, "spi-rx-bus-width", &rx_bus_width);
	if (ret < 0) {
	...
	ret = dev_read_u32(chip->dev, "spi-tx-bus-width", &tx_bus_width);
	if (ret < 0) {
	...
	quad_read_not_need_enable = dev_read_string(chip->dev,
			"read_from_cache_x4_not_need_enable");
	if (!quad_read_not_need_enable) {
	...
	read_seq_need_onedummy = dev_read_string(chip->dev,
			"read_from_cache_need_onedummy");
	if (!read_seq_need_onedummy) {
	...
	ret = dev_read_u32_default(chip->dev, "ecc_flag", -1);
	if (ret < 0) {
	...
	ret = dev_read_u32(chip->dev, "ecc_status_shift", &(info.ecc_status_shift));
	if (ret < 0) {
