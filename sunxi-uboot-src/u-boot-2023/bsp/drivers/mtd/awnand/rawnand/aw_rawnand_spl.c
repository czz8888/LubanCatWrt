		...
		unsigned int len, void *buf)
{
	...
	for (blk = start; blk < end; blk++) {
		...
		for (; page < ((blk << chip->pages_per_blk_shift) + pages_per_blk);
			...
			ret = chip->read_page(mtd, chip, mdata, chip->pagesize, sdata,
		...
		unsigned int len, void *buf)
{
	...
	for (blk = start; blk < end; blk++) {
		...
		for (; page < ((blk << chip->pages_per_blk_shift) + pages_per_blk);
			...
			ret = chip->write_page(mtd, chip, mdata, chip->pagesize, sdata, chip->avalid_sparesize, page);
			if (ret) {
...
void rawnand_download_boot_param(void)
{
	struct aw_nand_chip *chip = get_rawnand();
	int pages_per_blk = (1 << chip->pages_per_blk_shift);
	...
	boot_nand_para_t *boot_info = (boot_nand_para_t *)boot_param->nand_info;

	rawnand_mtd_get_flash_info(boot_info, BOOT_PARAM_SIZE);

	boot_param->header.check_sum = sunxi_generate_checksum(boot_param,
			sizeof(struct sunxi_boot_param_region), 1,
			boot_param->header.check_sum);

	rawnand_uboot_blknum(NULL, &boot_param_entry);

	boot_param_entry += AW_RAWNAND_RESERVED_PHY_BLK_FOR_SECURE_STORAGE;
	aw_rawnand_phy_write(boot_param_entry * block_size, BOOT_PARAM_SIZE >> 9, gd->sunxi_boot_param_addr);
...
int rawnand_mtd_download_boot0(unsigned int len, void *buf)
{
	...
	uint8_t *rbuf = malloc(ALIGN(len, 64));
	if (!rbuf)
...
int aw_rawnand_phy_write(unsigned int start, unsigned int sectors, void *buf)
{
	...
	align_buf = malloc(ALIGN(block_size, 64));
	if (!align_buf) {
