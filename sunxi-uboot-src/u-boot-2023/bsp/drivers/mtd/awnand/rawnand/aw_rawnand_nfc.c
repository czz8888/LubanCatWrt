...
#define MBUS_GATE           0x05E0
#define NAND0_CFG           0x0C80
#define NAND1_CFG           0x0C84
#define NAND_GATE           0x0C8C
#else
...
static void sunxi_nand_dma_cb_rx(void *p_arg)
{
	awrawnand_dbg("dma rx callback\n");
}

static void sunxi_nand_dma_cb_tx(void *p_arg)
{
	awrawnand_dbg("dma tx callback\n");
}

static int sunxi_nand_request_dma(struct aw_nand_host *host)
{
	struct uclass *uc;
	struct udevice *dev;
	int ret = 0;
	...
	if (ret) {
		awrawnand_err("get dma uclass failed %d\n", ret);
		return -EINVAL;
	}
	...
	uclass_foreach_dev(dev, uc) {
		...
		if (ret)
			awrawnand_warn("dma device probe failed %d\n", ret);
	}

	ret = sunxi_dma_request(&host->dma_tx);
	if (ret) {
		awrawnand_err("failed to request dma tx channel %d\n", ret);
		goto err0;
	}

	ret = sunxi_dma_request(&host->dma_rx);
	if (ret) {
		awrawnand_err("failed to request dma rx channel %d\n", ret);
		goto err1;
	}

	awrawnand_info("request dma channel %#lx(tx) and %#lx(rx) success\n", host->dma_tx.id, host->dma_rx.id);

	return 0;

err1:
	sunxi_dma_free(&host->dma_tx);
err0:
	return ret;
}

static void nand_dma_recv_start(struct aw_nand_host *host, uchar *pbuf, uint byte_cnt)
{
	...
	/* user normal DMA */
	reg_val = readl(nfc->ctl);
	reg_val |= (0x1U << 14 | 0x1U << 15);
	writel(reg_val, nfc->ctl);
	...
	sunxi_dma_install_int(host->dma_rx.id, sunxi_nand_dma_cb_rx, host);
	sunxi_dma_enable(&host->dma_rx);
	sunxi_dma_setting(host->dma_rx.id, &dma_set);
	sunxi_dma_start(host->dma_rx.id, (ulong)(SUNXI_NAND_BASE+NFC_REG_IO_DATA), (ulong)pbuf, byte_cnt);
}

static void nand_dma_send_start(struct aw_nand_host *host, uchar *pbuf, uint byte_cnt)
{
	sunxi_dma_set dma_set;
	__u32 reg_val;
	struct nfc_reg *nfc = &host->nfc_reg;
	...
	sunxi_dma_install_int(host->dma_tx.id, sunxi_nand_dma_cb_tx, host);
	sunxi_dma_enable(&host->dma_tx);
	sunxi_dma_setting(host->dma_tx.id, &dma_set);
	sunxi_dma_start(host->dma_tx.id, (ulong)pbuf, (ulong)(SUNXI_NAND_BASE+NFC_REG_IO_DATA), byte_cnt);
}

static int nand_wait_dma_recv_over(struct aw_nand_host *host)
{
	return sunxi_dma_querystatus(host->dma_rx.id);
}

static int nand_wait_dma_send_over(struct aw_nand_host *host)
{
	return sunxi_dma_querystatus(host->dma_tx.id);
}

static int wait_NDMA_recv_finish(struct aw_nand_host *host)
{
	ulong ctime = 0;
	int ret = 0;
	...
	ctime = timer_get_us();
	while (1) {
		if (nand_wait_dma_recv_over(host) == 0) {
			ret = 0;
			break;
		}
...
static int wait_NDMA_send_finish(struct aw_nand_host *host)
{
	ulong ctime = 0;
	int ret = 0;
	...
	ctime = timer_get_us();
	while (1) {
		if (nand_wait_dma_send_over(host) == 0) {
			ret = 0;
			break;
		}
		if (timer_get_us() - ctime > 0x4C4B40) {
			printf("rx wait_dma_send_over fail\n");
			ret = -1;
			break;
		}
	}
...
static int aw_host_nfc_wait_cmd_fifo_empty(struct nfc_reg *nfc)
{
	...
	if (ret) {
		awrawnand_err("wait cmd fifo empty timeout 1s status@%x %x\n",
				(unsigned int)nfc->sta, (unsigned int)readl(nfc->sta));
		aw_nfc_reg_dump(nfc);
	}
out:
	return ret;
...
static int aw_host_nfc_wait_status_timeout(struct nfc_reg *nfc, uint32_t mark, uint32_t value, uint32_t timeout_ms)
{
	...
	if (ret)
		awrawnand_err("wait nfc wait status 10s timeout[%x:%x:%x:%x]\n",
				mark, value, (unsigned int)nfc->sta, (unsigned int)readl(nfc->sta));
out:
...
static int aw_host_nfc_dma_config_start(struct aw_nand_host *host, uint8_t rw, void *addr, unsigned int len)
{
	...
	if (host->dma_type == MBUS_DMA) {
		...
		host->nfc_dma_desc_cpu[0].bcnt |= (__u32)(NFC_DESC_BSIZE(len) >> NDFC_DESC_ALIGNMENT);
		host->nfc_dma_desc_cpu[0].buff = (unsigned int)(host->dma_addr >> NDFC_DESC_ALIGNMENT);
		...
		host->nfc_dma_desc_cpu[0].next = (unsigned int)&host->nfc_dma_desc_cpu[0] >> NDFC_DESC_ALIGNMENT;

		config_addr = dma_map_single(&host->nfc_dma_desc_cpu[0], 1024, DMA_TO_DEVICE);
		if (host->use_dma_int)
			aw_host_nfc_dma_int_enable(&host->nfc_reg);

		writel(config_addr >> NDFC_DESC_ALIGNMENT, nfc->mbus_dma_dlba);
	} else {
		cfg = readl(nfc->dma_cnt);
		cfg &= ~0xffff;
		cfg |= (len & (0xFFFF));
		writel(cfg, nfc->dma_cnt);
		if (rw == READ) {
			nand_dma_recv_start(host, (uchar *)addr, len);
		} else {
			nand_dma_send_start(host, (uchar *)addr, len);
		}
	}
...
static int aw_host_nfc_batch_op(struct aw_nand_chip *chip, struct aw_nfc_batch_req *req)
{
	...
	if (req->data.type != ONLY_SPARE) {
		...
		if (host->dma_type == GENERIC_DMA) {
			if (req->type == READ) {
				ret |= wait_NDMA_recv_finish(host);
			} else {
				ret |= wait_NDMA_send_finish(host);
			}
		}
		if (ret) {
...
static inline int aw_host_resource_init(struct aw_nand_host *host)
{
	...
	host->mcclk_src = PLL_PERI0_1X;
#endif
	host->mcclk = (void __iomem *)SUNXI_CCM_BASE + NAND1_CFG;
	host->mbus_gate = (void __iomem *)SUNXI_CCM_BASE + MBUS_GATE;
	host->mdclk_val = 10;
	host->mcclk_val = 20;
#if defined(CONFIG_AW_MTD_RAWNAND_USE_GENERAL_DMA)
	host->dma_type = GENERIC_DMA;
#elif defined(CONFIG_AW_MTD_RAWNAND_USE_MBUS_DMA)
	...
	host->nfc_dma_desc_cpu = malloc(ALIGN(
...
int aw_host_init(struct udevice *dev)
{
	...
	if (host->dma_type == GENERIC_DMA) {
		if (sunxi_nand_request_dma(host)) {
			awrawnand_err("rawnand get general dma failed, change to internal dma\n");
			host->dma_type = MBUS_DMA;
		}
	}
