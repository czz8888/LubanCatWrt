...
__weak int fdt_getprop_string(const void *fdt, int nodeoffset,
...
int get_mtd_num_by_name(char *name)
{
	int i;
	struct ubi_info *ubinfo = get_ubi_info();
...
#ifdef CONFIG_AW_MTD_SPINAND
	if (spinand_dev && spr_sunxi_flash->parent == spinand_dev)
		ret = spinand_init_mtd_info(mtd_info);
#endif
#ifdef CONFIG_AW_MTD_RAWNAND
	if (rawnand_dev && spr_sunxi_flash->parent == rawnand_dev)
...
static int create_ubi_volume(struct ubi_info *ubinfo)
{
	 ...
	 */
	check_and_adjust_ubi_mbr(&ubi_mbr->vols[i - 1], nand_mbr, nand_mbr->part_cnt - 1);
	check_and_adjust_ubi_mbr(&ubi_mbr->vols[i -1], ubi_mbr, ubi_mbr->part_cnt - 1);
...
check0:
		...
		read = mtd->erasesize - (from & mtd->erasesize_mask);
		ret = read_mtd_do(read_start, read, &retlen, buffer);
		if (ret && ret != -EUCLEAN) {
			pr_err("read from %u len %u err\n", from, len);
			return ret;
		}
...
uint64_t spinand_sys_part_offset(void)
{
	...
	if (fdt_path_offset(working_fdt, "/soc/sunxi_flashmap/nand_map") < 0) {
	...
	if (CONFIG_KERNEL_SIZE_BYTE) {
		...
		char fdt_node_name[NAND_MAP_PATH_SIZE] = {'0'};
		int fdt_off;

		while (1) {
			snprintf(fdt_node_name, NAND_MAP_PATH_SIZE,
...
static void set_env_mtdparts_spinand(void)
{
	...
	if (fdt_path_offset(working_fdt, "/soc/sunxi_flashmap/nand_map") < 0) {
		pr_warn("The partition nonsupport be initialized using the device tree\n");
/*boot mtd part append to uboot mtd part tail, boot0 is easy to handle*/
	...
	wlen += snprintf(ubinfo->mtdparts + wlen, 512 - wlen,
		...
		while (1) {
			...
			snprintf((char *)&mtd_part.name, PART_NAME_MAX_SIZE, part_name);

			mtd_part.offset = offset;
			fdt_getprop_u32(working_fdt, fdt_off, "phy_block_num", &mtd_part.bytes);
			mtd_part.bytes *= phy_blk_bytes;
			offset += mtd_part.bytes;
			wlen += snprintf(ubinfo->mtdparts + wlen, 512 - wlen,
				"%uk@%u(%s),", mtd_part.bytes / SZ_1K,
...
int spinand_ubi_attach_mtd(void)
{
	int num;
	struct ubi_info *ubinfo = get_ubi_info();
	struct ubi_mtd_info *mtd_info = ubi_to_mtd(ubinfo);
...
#ifdef CONFIG_AW_MTD_RAWNAND
uint64_t rawnand_sys_part_offset(void)
{
	struct aw_nand_chip *chip = get_rawnand();
	...
	offset += AW_RAWNAND_RESERVED_PHY_BLK_FOR_SECURE_STORAGE * phy_blk_bytes;
...
int rawnand_ubi_attach_mtd(void)
{
	int num;
	struct ubi_info *ubinfo = get_ubi_info();
	struct ubi_mtd_info *mtd_info = ubi_to_mtd(ubinfo);
