...
int arisc_dvfs_cfg_vf_table(void)
{
	...
	if (IS_ERR_VALUE(fdt_getprop_u32_arisc(working_fdt, nodeoffset, "vf_table_count", &vf_table_count))) {
	...
	if (IS_ERR_VALUE(fdt_getprop_u32_arisc(working_fdt, nodeoffset, "lv_count", &vf_table_size))) {
	...
	for (index = 0; index < vf_table_size; index++) {
		...
		if (!IS_ERR_VALUE(fdt_getprop_u32_arisc(working_fdt, nodeoffset, vf_table_sub_key, &value))) {
...
static int sunxi_arisc_parse_cfg(void)
{
	...
	if (IS_ERR_VALUE(fdt_getprop_u32_arisc(working_fdt, nodeoffset, "space4", value))) {
	...
	ARISC_INF("arisc_space space4 msgpool_dst:0x%p, msgpool_offset:0x%p, msgpool_size:0x%zx, \n",
		(void *)(ulong)dts_cfg.space.msgpool_dst, (void *)(ulong)dts_cfg.space.msgpool_offset, dts_cfg.space.msgpool_size);
	...
	if (IS_ERR_VALUE(fdt_getprop_u32_arisc(working_fdt, nodeoffset, "reg", value))) {
	...
	ARISC_INF("msgbox base:0x%p, size:0x%zx, status:%u\n",
		(void *)(ulong)dts_cfg.msgbox.base, dts_cfg.msgbox.size, dts_cfg.msgbox.status);
	...
	ARISC_INF("hwspinlock base:0x%p, size:0x%zx, status:%u\n",
		(void *)(ulong)dts_cfg.hwspinlock.base, dts_cfg.hwspinlock.size, dts_cfg.hwspinlock.status);
...
/* 	memcpy((void *)&dts_cfg_64.dram_para, (const void *)&dts_cfg.dram_para, sizeof(struct dram_para));
	...
	memcpy((void *)&dts_cfg_64.pmu, (const void *)&dts_cfg.pmu,
	       sizeof(struct pmu_cfg));
	memcpy((void *)&dts_cfg_64.power, (const void *)&dts_cfg.power,
	       sizeof(struct power_cfg));
	flush_dcache_range((unsigned long)ALIGN_DOWN((u32)&dts_cfg_64, CONFIG_SYS_CACHELINE_SIZE),
			   (unsigned long)ALIGN((u32)&dts_cfg_64 +
						 sizeof(struct dts_cfg_64), CONFIG_SYS_CACHELINE_SIZE)); */
}
