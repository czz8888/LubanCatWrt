...
static int sunxi_pcie_rd_other_conf(struct sunxi_pcie_port *pp, pci_dev_t d, int where, int size, ulong *val)
{
	...
	sunxi_pcie_prog_outbound_atu(pp, PCIE_ATU_INDEX0, type, pp->cpu_cfg0_base, busdev, pp->cfg0_size);
...
{
	struct sunxi_pcie *pcie = dev_get_plat(bus);
	int ret, size_len = 4;
...
static void sunxi_pcie_host_setup_rc(struct sunxi_pcie_port *pp)
{
	...
	if (pp->cpu_pcie_addr_quirk)
		mem_base = pp->cpu_mem_base - PCIE_CPU_BASE;
	else
		mem_base = pp->cpu_mem_base;

	sunxi_pcie_prog_outbound_atu(pp, PCIE_ATU_INDEX1, PCIE_ATU_TYPE_MEM, mem_base,
						pp->pcie_mem_base,
						pp->mem_size);

	sunxi_pcie_prog_outbound_atu(pp, PCIE_ATU_INDEX2, PCIE_ATU_TYPE_IO, pp->cpu_io_base,
						pp->pcie_io_base,
						pp->io_size);
...
static void sunxi_pcie_host_init(struct udevice *dev)
{
	...
	if (dm_gpio_is_valid(&pci->wake_gpio)) {
		dm_gpio_set_value(&pci->wake_gpio, 1);
	}
...
static int sunxi_pcie_parse_rc_dts(struct udevice *dev)
{
	ofnode node = dev_ofnode(dev);
	const fdt32_t *ranges;
	int rangesz = 7, nranges, flags;
    int len, i;
	u64 res_cpuaddr, res_pcieaddr, res_size;
	...
	if (!ranges) {
		printf("No 'ranges' property found\n");
		return -EINVAL;
    }

	nranges = len / sizeof(u32) / rangesz;

	for (i = 0; i < nranges; i++, ranges += rangesz) {
		flags = of_read_number(ranges, 1);
		res_pcieaddr = of_read_number(ranges + 1, 2);
		res_cpuaddr = of_read_number(ranges + 3, 2);
		res_size = of_read_number(ranges + 5, 2);

		if (DT_FLAGS_TO_TYPE(flags) == DT_RANGE_TYPE_CFG) {
			pci->pcie_port.cpu_cfg0_base = res_cpuaddr;
			pci->pcie_port.pcie_cfg0_base = res_pcieaddr;
			pci->pcie_port.cfg0_size = res_size;
			pci->pcie_port.va_cfg0_base = (void *)pci->pcie_port.cpu_cfg0_base;
		} else if (DT_FLAGS_TO_TYPE(flags) == DT_RANGE_TYPE_IO) {
			pci->pcie_port.cpu_io_base = res_cpuaddr;
			pci->pcie_port.pcie_io_base = res_pcieaddr;
			pci->pcie_port.io_size = res_size;
		} else if (DT_FLAGS_TO_TYPE(flags) == DT_RANGE_TYPE_MEM32 || DT_FLAGS_TO_TYPE(flags) == DT_RANGE_TYPE_MEM64) {
			pci->pcie_port.cpu_mem_base = res_cpuaddr;
			pci->pcie_port.pcie_mem_base = res_pcieaddr;
			pci->pcie_port.mem_size = res_size;
		}
	}
...
{
	struct sunxi_pcie *pcie = dev_get_plat(dev);
	...
	sunxi_pcie_parse_rc_dts(dev);

	/* PCI I/O space */
	pci_set_region(&hose->regions[0],
			pcie->pcie_port.pcie_io_base,
			pcie->pcie_port.cpu_io_base,
			pcie->pcie_port.io_size,
			PCI_REGION_IO);

	/* System memory space */
	pci_set_region(&hose->regions[1],
			pcie->pcie_port.pcie_mem_base,
			pcie->pcie_port.cpu_mem_base,
			0xffffffff, //TODO
			PCI_REGION_MEM);

	hose->region_count = 2;

	ret = sunxi_pcie_plat_hw_init(pcie);
...
static int sunxi_pci_rc_remove(struct udevice *dev)
{
	return 0;
}
...
static const struct udevice_id sunxi_pcie_ids[] = {
	...
	},
	{
		...
		.data = (ulong)&sunxi_pcie_rc_v210_v2_of_data,
	},
	{
