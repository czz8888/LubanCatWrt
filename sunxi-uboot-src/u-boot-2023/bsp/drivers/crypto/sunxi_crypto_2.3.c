...
static int _sunxi_md5_calc(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	u32 src_align_len					     = 0;
	u32 total_bit_len					     = 0;
	...
	flush_cache((uintptr_t)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)p_sign, CACHE_LINE_SIZE);
	flush_cache((uintptr_t)src_addr, ALIGN(src_align_len, CACHE_LINE_SIZE));

	ss_set_drq((uintptr_t)&task0);
...
int sunxi_hash_test(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len, u32 sha_type)
{
	...
	flush_cache(((uintptr_t)&task0), ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)p_sign, CACHE_LINE_SIZE);
	flush_cache(((uintptr_t)src_addr), ALIGN(src_len, CACHE_LINE_SIZE));
...
static int _sunxi_sha_calc(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	u32 total_bit_len					     = 0;
	struct hash_task_descriptor task0 __aligned(CACHE_LINE_SIZE) = { 0 };
...
{
	return sunxi_sha_calc_dm(dst_addr, dst_len, src_addr, src_len);
}
		     ...
		     u32 dst_len, u8 *src_addr, u32 src_len)
{
	...
	flush_cache((uintptr_t)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)p_n, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)p_e, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)p_src, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)p_dst, ALIGN(mod_size_len_inbytes, CACHE_LINE_SIZE));
			    ...
			    uint32_t symmetric_ctl, uint32_t c_ctl)
{
	...
	if ((unsigned long)src_addr & (CACHE_LINE_SIZE - 1) || (unsigned long)dst_addr & (CACHE_LINE_SIZE - 1)) {
		printf("%s cache misaligned\n", __func__);
	}
	...
	//flush&clean cache
	flush_cache((uintptr_t)iv, ALIGN(sizeof(iv), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)key_map, ALIGN(sizeof(key_map), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)src_addr, ALIGN(len, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)dst_addr, ALIGN(len, CACHE_LINE_SIZE));
		     ...
		     u8 *p_tmp, u8 *hash, u32 hash_byte_len)
{
	...
	if (!memcmp(params->r, all_zero, params->r_len) ||
		...
		return -1;
	//n
	__rsa_padding(tmp_p, params->n, params->n_len, params->n_len);
	tmp_p += key_byte_size;

	//s
	__rsa_padding(tmp_p, params->s, params->s_len, params->s_len);
	tmp_p += key_byte_size;

	//hash
	__rsa_padding(tmp_p, hash, hash_byte_len, hash_byte_len);

	tmp_p += key_byte_size;

	//r
	__rsa_padding(tmp_p, params->r, params->r_len, params->r_len);
	tmp_p += key_byte_size;

	//p
	__rsa_padding(tmp_p, params->p, params->p_len, params->p_len);
	tmp_p += key_byte_size;

	//a
	__rsa_padding(tmp_p, params->a, params->a_len, params->a_len);
	tmp_p += key_byte_size;

	//gx
	__rsa_padding(tmp_p, params->gx, params->gx_len, params->gx_len);
	tmp_p += key_byte_size;

	//gy
	__rsa_padding(tmp_p, params->gy, params->gy_len, params->gy_len);
	tmp_p += key_byte_size;

	//qx
	__rsa_padding(tmp_p, params->qx, params->qx_len, params->qx_len);
	tmp_p += key_byte_size;

	//qy
	__rsa_padding(tmp_p, params->qy, params->qy_len, params->qy_len);
	tmp_p += key_byte_size;
...
#define BUFF_LEN (68)
	...
	memset((void *)(&task0), 0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	memset((void *)(temp_src_addr), 0, BUFF_LEN * 12);
	memset((void *)(temp_dst_addr), 0, BUFF_LEN);

	tmp_src = (uintptr_t)temp_src_addr;
	tmp_dst = (uintptr_t)temp_dst_addr;
	p_tmp = temp_src_addr;
	...
	flush_cache((uintptr_t)&task0,  ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)temp_src_addr, ALIGN(BUFF_LEN * 12, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)temp_dst_addr, ALIGN(BUFF_LEN, CACHE_LINE_SIZE));
...
#define MAX_RSA_BUFF_LEN ((4096 >> 3) + 32)
	...
	ALLOC_CACHE_ALIGN_BUFFER(u8, temp_key_addr, ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	ALLOC_CACHE_ALIGN_BUFFER(u8, temp_src_addr, ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	ALLOC_CACHE_ALIGN_BUFFER(u8, temp_dst_addr, ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	ALLOC_CACHE_ALIGN_BUFFER(u8, temp_mod_addr, ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	...
	tmp_key = (uintptr_t)temp_key_addr;
	tmp_src = (uintptr_t)temp_src_addr;
	tmp_dst = (uintptr_t)temp_dst_addr;
	tmp_mod = (uintptr_t)temp_mod_addr;
	...
	flush_cache((uintptr_t)&task0, ALIGN(sizeof(task0), CACHE_LINE_SIZE));
	flush_cache((uintptr_t)temp_key_addr,
		    ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)temp_src_addr,
		    ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)temp_dst_addr,
		    ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
	flush_cache((uintptr_t)temp_mod_addr,
		    ALIGN(MAX_RSA_BUFF_LEN, CACHE_LINE_SIZE));
...
int sunxi_md5_calc_dm(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	int ret;
	struct udevice *dev_hash = NULL;

	ret = uclass_get_device_by_driver(UCLASS_HASH,
		DM_DRIVER_GET(sunxi_ss_hash), &dev_hash);
	if (ret) {
		pr_err("Unable to find device: sunxi_ss_hash, %d\n", ret);
		return ret;
	}

	ret = hash_digest(dev_hash, HASH_ALGO_SHA256, (void *)src_addr, src_len, (void *)dst_addr);
	if (ret) {
		pr_err("sunxi_ss_hash hash_digest failed: %d\n", ret);
	}
...
{
	return sunxi_md5_calc_dm(dst_addr, dst_len, src_addr, src_len);
}

int sunxi_sha_calc_dm(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	int ret;
	struct udevice *dev_hash = NULL;
...
s32 _sunxi_rsa_calc(u8 *n_addr, u32 n_len, u8 *e_addr, u32 e_len, u8 *dst_addr,
		   u32 dst_len, u8 *src_addr, u32 src_len)
{
	const u32 TEMP_BUFF_LEN = ALIGN((2048 >> 3), CACHE_LINE_SIZE);
...
s32 sunxi_rsa_calc_dm(u8 *n_addr, u32 n_len, u8 *e_addr, u32 e_len, u8 *dst_addr,
		   u32 dst_len, u8 *src_addr, u32 src_len)
{
	int ret;
	struct udevice *dev_rsa = NULL;

	ret = uclass_get_device_by_driver(UCLASS_MOD_EXP,
		DM_DRIVER_GET(sunxi_ss_rsa), &dev_rsa);
	if (ret) {
		pr_err("Unable to find device: sunxi_ss_rsa, %d\n", ret);
		return ret;
	}
	...
	prop.modulus = (void *)(n_addr);
	prop.public_exponent = (void *)e_addr;
	prop.num_bits = n_len * 8;
	prop.exp_len = e_len;

	ret = rsa_mod_exp(dev_rsa, src_addr, src_len, &prop, dst_addr);
	if (ret) {
		pr_err("sunxi_ss_rsa rsa_mod_exp failed: %d\n", ret);
	}

	return ret;
}

int sunxi_rsa_calc(u8 *n_addr, u32 n_len, u8 *e_addr, u32 e_len, u8 *dst_addr,
		u32 dst_len, u8 *src_addr, u32 src_len)
{
	return sunxi_rsa_calc_dm(n_addr, n_len, e_addr, e_len, dst_addr, dst_len,
					src_addr, src_len);
}

static int sunxi_ss_rsa_mod_exp(struct udevice *dev, const uint8_t *sig, uint32_t sig_len,
					struct key_prop *prop, uint8_t *out)
{

	return _sunxi_rsa_calc((u8 *)prop->modulus, prop->num_bits / 8,
			(u8 *)prop->public_exponent, prop->exp_len, out, 32, (u8 *)sig, sig_len);
}

/* enable low power function */
static void ss_low_power_en(void)
{
	u32 reg_val;

	reg_val = readl(SS_N_LPC);
	reg_val |= (0x1 << CE_LOW_POWER_EN);
	writel(reg_val, SS_N_LPC);
}

static const struct mod_exp_ops sunxi_ss_rsa_ops = {
	.mod_exp = sunxi_ss_rsa_mod_exp,
};

static const struct udevice_id sunxi_ss_rsa_ids[] = {
	{ .compatible = "allwinner,sunxi-ce-rsa" },
	{ }
};

U_BOOT_DRIVER(sunxi_ss_rsa) = {
	.name = "sunxi_ss_rsa",
	.id = UCLASS_MOD_EXP,
	.of_match = sunxi_ss_rsa_ids,
	.ops = &sunxi_ss_rsa_ops,
};

static int sunxi_ss_digest(struct udevice *dev, enum HASH_ALGO algo,
			const void *ibuf, const uint32_t ilen,
			void *obuf)
{
	/* only support sha256, md5*/
	switch (algo) {
	case HASH_ALGO_SHA256:
		return _sunxi_sha_calc((u8 *)obuf, 32, (u8 *)ibuf, ilen);
	case HASH_ALGO_MD5:
		return _sunxi_md5_calc((u8 *)obuf, 16, (u8 *)ibuf, ilen);
	default:
		pr_err("do not support algo: %d\n", algo);
		return -1;
	}
}

static const struct hash_ops sunxi_ss_hash_ops = {
	.hash_digest = sunxi_ss_digest,
};

static const struct udevice_id sunxi_ss_hash_ids[] = {
	{ .compatible = "allwinner,sunxi-ce-hash" },
	{ }
};

U_BOOT_DRIVER(sunxi_ss_hash) = {
	.name = "sunxi_ss_hash",
	.id = UCLASS_HASH,
	.of_match = sunxi_ss_hash_ids,
	.ops = &sunxi_ss_hash_ops,
};

static int sunxi_ss_resources_get(struct sunxi_ce_cdev *ce)
{
	int err;

	ce->reset_ce = devm_reset_control_get(ce->dev, "rst_ce");
	if (IS_ERR(ce->reset_ce)) {
		pr_err("ce get rst_ce_clk failed\n");
		return PTR_ERR(ce->reset_ce);
	}

	ce->reset_ce_sys = devm_reset_control_get(ce->dev, "rst_ce_sys");
	if (IS_ERR_OR_NULL(ce->reset_ce_sys)) {
		pr_debug("ce get rst_ce_sys_clk failed\n");
	}

	ce->src_clk = devm_clk_get(ce->dev, "src_clk");
	if (IS_ERR(ce->src_clk)) {
		pr_err("ce get source_clk failed\n");
		err = PTR_ERR(ce->src_clk);
		goto put_reset_ce_sys_clk;
	}

	ce->mod_clk = devm_clk_get(ce->dev, "mod_clk");
	if (IS_ERR(ce->mod_clk)) {
		pr_err("ce get module_clk failed\n");
		err = PTR_ERR(ce->mod_clk);
		goto put_src_clk;
	}

	ce->bus_clk = devm_clk_get(ce->dev, "bus_clk");
	if (IS_ERR(ce->bus_clk)) {
		pr_err("ce get bus_clk failed\n");
		err = PTR_ERR(ce->bus_clk);
		goto put_mod_clk;
	}

	ce->mbus_clk = devm_clk_get(ce->dev, "mbus_clk");
	if (IS_ERR(ce->mbus_clk)) {
		pr_err("ce get mbus_clk failed\n");
		err = PTR_ERR(ce->mbus_clk);
		goto put_bus_clk;
	}

	ce->sys_clk = devm_clk_get(ce->dev, "sys_clk");
	if (IS_ERR(ce->sys_clk)) {
		pr_err("ce get sys_clk failed\n");
		err = PTR_ERR(ce->sys_clk);
		goto put_mbus_clk;
	}
...
put_mbus_clk:
	devm_clk_put(ce->dev, ce->mbus_clk);
put_bus_clk:
	devm_clk_put(ce->dev, ce->bus_clk);
put_mod_clk:
	devm_clk_put(ce->dev, ce->mod_clk);
put_src_clk:
	devm_clk_put(ce->dev, ce->src_clk);
put_reset_ce_sys_clk:
	if (!IS_ERR_OR_NULL(ce->reset_ce_sys))
		reset_free(ce->reset_ce_sys);
	reset_free(ce->reset_ce);
	return err;
}

static void sunxi_ss_resources_put(struct sunxi_ce_cdev *ce)
{
	devm_clk_put(ce->dev, ce->sys_clk);
	devm_clk_put(ce->dev, ce->mbus_clk);
	devm_clk_put(ce->dev, ce->bus_clk);
	devm_clk_put(ce->dev, ce->mod_clk);
	devm_clk_put(ce->dev, ce->src_clk);
...
static int sunxi_ss_clk_init(struct sunxi_ce_cdev *ce)
{
	int err;

	reset_assert(ce->reset_ce);
	if (!IS_ERR_OR_NULL(ce->reset_ce_sys))
		reset_assert(ce->reset_ce_sys);

	udelay(2);

	if (!IS_ERR_OR_NULL(ce->reset_ce_sys))
		reset_deassert(ce->reset_ce_sys);
	reset_deassert(ce->reset_ce);

	err = clk_prepare_enable(ce->bus_clk);
	if (err < 0) {
		pr_err("ce enable bus_clk failed: %d\n", err);
		goto assert_ret_clk;
	}

	err = clk_prepare_enable(ce->src_clk);
	if (err < 0) {
		pr_err("ce enable src_clk failed: %d\n", err);
		goto disable_bus_clk;
	}

	err = clk_set_parent(ce->mod_clk, ce->src_clk);
	if (err < 0) {
		pr_err("ce enable src_clk failed: %d\n", err);
		goto disable_bus_clk;
	}

	err = clk_prepare_enable(ce->mod_clk);
	if (err < 0) {
		pr_err("ce enable mod_clk failed: %d\n", err);
		goto disable_src_clk;
	}

	err = clk_prepare_enable(ce->mbus_clk);
	if (err < 0) {
		pr_err("ce enable mbus_clk failed: %d\n", err);
		goto disable_mod_clk;
	}

	err = clk_prepare_enable(ce->sys_clk);
	if (err < 0) {
		pr_err("ce enable sys_clk failed: %d\n", err);
		goto disable_mbus_clk;
	}

	ce->low_power_mode = dev_read_bool(ce->dev, "low-power-mode");
	if (ce->low_power_mode)
		ss_low_power_en();

	return 0;

disable_mbus_clk:
	clk_disable_unprepare(ce->mbus_clk);
disable_mod_clk:
	clk_disable_unprepare(ce->mod_clk);
disable_src_clk:
	clk_disable_unprepare(ce->src_clk);
disable_bus_clk:
...
static void sunxi_ss_clk_exit(struct sunxi_ce_cdev *ce)
{
	clk_disable_unprepare(ce->sys_clk);
	clk_disable_unprepare(ce->mbus_clk);
	clk_disable_unprepare(ce->mod_clk);
	clk_disable_unprepare(ce->src_clk);
	clk_disable_unprepare(ce->bus_clk);
...
static int sunxi_ss_bind(struct udevice *dev)
{
	struct sunxi_ce_cdev *ce = dev_get_priv(dev);
	int err;
	...
	err = sunxi_ss_resources_get(ce);
	if (err) {
		pr_err("ce get resources failed\n");
		goto free_malloc;
	}

	err = sunxi_ss_clk_init(ce);
	if (err) {
		pr_err("ce clock init failed\n");
		goto resources_put;
	}
...
resources_put:
	sunxi_ss_resources_put(ce);
free_malloc:
	free(ce);
	return err;
}

static int sunxi_ss_probe(struct udevice *dev)
{
	sunxi_ss_bind(dev);
...
static int sunxi_ss_remove(struct udevice *dev)
{
	struct sunxi_ce_cdev *ce = dev_get_priv(dev);

	sunxi_ss_clk_exit(ce);
...
static const struct udevice_id sunxi_ss_ids[] = {
	{ .compatible = "allwinner,sunxi-ce" },
	{ }
};

U_BOOT_DRIVER(sunxi_ss) = {
	.name		= "sunxi_ss",
	.id		= UCLASS_MISC,
	.of_match	= sunxi_ss_ids,
	.probe		= sunxi_ss_probe,
	.remove         = sunxi_ss_remove,
	.bind		= sunxi_ss_bind,
	.priv_auto	= sizeof(struct sunxi_ce_cdev),
};
