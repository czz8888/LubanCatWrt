...
int sunxi_codec_write(void __iomem *addr_base, u32 reg, u32 val)
{
	codec_wrreg(addr_base + reg, val);
	return 0;
}

int sunxi_codec_update_bits(void __iomem *addr_base, u32 reg, u32 mask,
				   u32 val)
{
	codec_wrreg_bits(addr_base + reg, mask, val);
	return 0;
}
