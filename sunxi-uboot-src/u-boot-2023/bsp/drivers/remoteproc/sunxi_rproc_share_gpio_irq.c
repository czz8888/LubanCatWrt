...
/*
 * drivers/remoteproc/sunxi_rproc_share_gpio_irq.c
...
#include "sunxi_rproc_share_gpio_irq.h"

#define UART_NUM (6)

int fdt_getprop_u32_value(const void *fdt, int nodeoffset,
		     ...
		     int *nodeoffset)
{
	...
	SUNXI_RPROC_TRACE("riscv:---- name [%s] msg -----\n", path);
	nodeoff = fdt_path_offset(working_fdt, path);
	if (nodeoff < 0) {
		SUNXI_RPROC_TRACE("riscv:find [%s] node err\n", path);
		return -1;
	}
	...
	if (pstr != NULL) {
		...
		if (strcmp(pstr, compatible_str) == 0) {
			*nodeoffset = nodeoff;
			...
			SUNXI_RPROC_TRACE("riscv:expect compatible:[%s], but use compatible:[%s]\n",\
						compatible_str, pstr);
			return -1;
		}

	} else {
		SUNXI_RPROC_TRACE("riscv:find compatible err\n");
		return -1;
	}
...
int dts_riscv_status(struct dts_msg_t *pmsg, u32 riscv_id)
{
	...
	if (ret < 0) {
		SUNXI_RPROC_TRACE("riscv%d:dts no config [%s]\n", riscv_id, str);
		return -1;
	}

	SUNXI_RPROC_TRACE("riscv%d:dts can find config [%s]\n", riscv_id, str);

	ret = match_status(nodeoff);
	if (ret < 0) {
		pmsg->riscv_status = DTS_CLOSE;
		SUNXI_RPROC_TRACE("%s status is close\n", str);
		return -1;
	}
...
int riscv_dts_gpio_int_msg(struct dts_msg_t *pmsg, u32 riscv_id)
{
	...
	if (ret%3 != 0) {
		SUNXI_RPROC_TRACE("riscv%d:[%s] format err\n", riscv_id, str);
		return -1;
	}
	ret /= 3;
...
int riscv_dts_sharespace_msg(struct dts_msg_t *pmsg, u32 riscv_id)
{
	...
	ret = fdt_getprop_u32_value(working_fdt, nodeoff, "reg", reg_data);
	if (ret < 0) {
	...
	for (i = 0; i < 8; i++) {
		SUNXI_RPROC_TRACE("riscv%d:dts reg[%d]=0x%x\n", riscv_id, i, reg_data[i]);
	}
