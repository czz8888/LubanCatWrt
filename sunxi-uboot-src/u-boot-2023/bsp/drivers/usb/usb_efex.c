...
#if defined(CONFIG_AW_MTD_SPINAND)
#include <linux/mtd/aw-spinand.h>
#endif
#if defined(CONFIG_AW_MTD_RAWNAND)
...
static int sunxi_usb_efex_write_enable;
...
static  u8  *cmd_buf;
static u32 sunxi_efex_next_action;
static  struct pmu_config_t  pmu_config;
static  struct multi_unseq_mem_s global_unseq_mem_addr;
static int sunxi_usb_efex_usb3_status = 1;

static u32 dma_recv_time_out;
extern int do_bootelf(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[]);
int runtime_tick(void);

int efex_suspend_flag;
...
static int sunxi_efex_init(void)
{
    ...
    trans_data.base_recv_buffer = (u8 *)memalign(CONFIG_SYS_CACHELINE_SIZE, SUNXI_EFEX_RECV_MEM_SIZE);
	...
	trans_data.base_send_buffer = (u8 *)memalign(CONFIG_SYS_CACHELINE_SIZE, SUNXI_EFEX_RECV_MEM_SIZE);
...
static void  sunxi_efex_usb_rx_dma_isr(void *p_arg)
{
	...
	if (usb_is_dwc3()) {
		struct usb_request *req = p_arg;
		sunxi_ubuf_t *sunxi_ubuf = (sunxi_ubuf_t *)get_sunxi_ubuf();

		memset(sunxi_ubuf->rx_req_buffer, 0, SUNXI_USB_REQ_BUFFER_LEN);
		memcpy((void *)sunxi_ubuf->rx_req_buffer, req->buf, req->actual);
		sunxi_ubuf->rx_req_length = req->actual;
...
static int __sunxi_usb_efex_op_cmd(u8 *cmd_buffer)
{
	...
	switch(cmd->app_cmd)
	{
		...
		case FEX_CMD_fes_up:
			...
			if ((get_boot_storage_type() == STORAGE_NAND || get_boot_storage_type() == STORAGE_SPI_NAND) &&
...
//				fes_cmd_verify_status_t *cmd_verify = (fes_cmd_verify_status_t *)cmd_buf;
		...
		case FEX_CMD_fes_flash_set_on:
			...
			trans_data.last_err = 0; //sunxi_sprite_init(0);
			trans_data.app_next_status   = SUNXI_USB_EFEX_APPS_STATUS;
		...
		case FEX_CMD_fes_flash_set_off:
			...
			trans_data.last_err = 0; //TODO sunxi_sprite_exit(1);
			trans_data.app_next_status   = SUNXI_USB_EFEX_APPS_STATUS;
		...
		case FEX_CMD_fes_unseqmem_read:
			{
				...
				if(global_unseq_mem_addr.unseq_mem == NULL)
				{
					...
					trans_data.last_err        = -1;
					trans_data.act_send_buffer = (void*)(ulong)CFG_SYS_SDRAM_BASE;
				}
				else
				{
					int i;
		...
		case FEX_CMD_fes_force_erase:
			printf("FEX_CMD_fes_force_erase\n");
			{
				trans_data.last_err = 0; //TODO sunxi_flash_force_erase();
...
#endif
		    trans_data.last_err = 0; //TODO sunxi_sprite_force_erase_key();
...
static void dram_data_recv_finish(uint data_type)
{
	...
	{
		if (trans_data.recv_size == 4 && (ulong)trans_data.dram_trans_buffer < CFG_SYS_SDRAM_BASE)
			writel((*(unsigned int *)trans_data.act_recv_buffer), (void __iomem *)(ulong)trans_data.dram_trans_buffer);
		else
...
void sunxi_print_efex_status(int efex_status)
{
	...
	if (usb_debug_mode) {
		if (temp_efex_status != efex_status || cnt < 5) {
			if (efex_status == temp_efex_status) {
				cnt++;
			} else {
				cnt = 0;
			}
...
static int sunxi_efex_state_loop(void  *buffer)
{
	...
	switch(sunxi_usb_efex_status)
	{
	  	...
	  	case SUNXI_USB_EFEX_RECEIVE_DATA:
			...
			if(sunxi_usb_efex_write_enable == 1)		//数据部分接收完毕
			{
				...
				if(sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_CMD)
				{
					...
					{
						if (usb_is_dwc3()) {
							//usb3 gadget无需在使用时区分cpu or dma，都用同一个接口搬
							memcpy(cmd_buf, sunxi_ubuf->rx_req_buffer, sunxi_ubuf->rx_req_length);
						}
		...
		if (usb_is_dwc3()) {
			...
			if (!recv_len)
				recv_buf = trans_data.act_recv_buffer;

			memcpy(recv_buf, sunxi_ubuf->rx_req_buffer, sunxi_ubuf->rx_req_length);
			recv_len += sunxi_ubuf->rx_req_length;
			recv_buf += sunxi_ubuf->rx_req_length;
			// sunxi_usb_dbg("cp rx_req_buffer(len:%d) to recv_buf,remain %d(%d) bytes\n",
			// 	sunxi_ubuf->rx_req_length, sunxi_ubuf->request_size - recv_len, sunxi_ubuf->request_size);

			if (recv_len < sunxi_ubuf->request_size) {
				// 还没接收完
				sunxi_usb_efex_write_enable   = 0;
				sunxi_ubuf->rx_ready_for_data = 0;
				break;
		...
		case SUNXI_USB_EFEX_STATUS:
			if (!usb_is_dwc3() || (usb_is_dwc3() && sunxi_usb_efex_usb3_status)) {
				sunxi_usb_efex_status = SUNXI_USB_EFEX_IDLE;
		...
		case SUNXI_USB_EFEX_EXIT:
			...
			if (usb_is_dwc3()) {
				if (sunxi_usb_efex_usb3_status == 1) {
					sunxi_usb_efex_usb3_status++;
					sunxi_ubuf->rx_ready_for_data = 0;

					__sunxi_efex_send_status(&csw, sizeof(struct sunxi_efex_csw_t));
				} else if (sunxi_usb_efex_usb3_status > 2) {
