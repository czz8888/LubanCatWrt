...
sunxi_key_ladder_verify_rotpk_hash(void *input_hash_buf, int len)
{
	pr_debug("call weak fun: %s\n", __func__);
	return -1;
}

__attribute__((weak)) int smc_set_sst_crypt_name(char *name)
{
	pr_debug("call weak fun: %s\n", __func__);
	return 0;
}

__attribute__((weak)) int sunxi_verify_rotpk_hash(void *input_hash_buf, int len)
{
	pr_debug("call weak fun: %s\n", __func__);
	return -1;
}

__attribute__((weak)) int sunxi_secure_object_down(const char *name, char *buf,
						   int len, int encrypt,
						   int write_protect)
...
static int sunxi_usb_pburn_write_enable;
...
static u8 *private_data_ext_buff_step;
static u8 *private_data_ext_buff;

long long burn_part_bytes;
...
static int __usb_get_descriptor(struct usb_device_request *req, uchar *buffer)
{
	...
	case USB_DT_DEVICE: //设备描述符
	{
		...
		memset((void *)dev_dscrptr, 0,
		       (u32)sizeof(struct usb_device_descriptor));
	...
	case USB_DT_DEVICE_QUALIFIER: {
		...
		memset(&buffer, 0,
		       (u32)sizeof(struct usb_qualifier_descriptor));
...
{
	sunxi_usb_burn_main_info_t *key_main =
		(sunxi_usb_burn_main_info_t *)buff;
	...
	if (strcmp((const char *)key_main->magic, "key-group-db")) {
		pr_err("key data magic unmatch, err\n");

		return -1;
	}
	key_count = key_main->count;
	pr_notice("key_count=%d\n", key_count);
	...
	for (; key_count > 0; key_count--, key_list++) {
		...
		pr_notice("^^^^^^^^^^^^^^^^^^^\n");
		pr_notice("key index=%d\n", key_main->count - key_count);
		pr_notice("key name=%s\n", key_list->name);
		pr_notice("key type=%d\n", key_list->type);
		pr_notice("key len=%d\n", key_list->len);
		pr_notice("key if_burn=%d\n", key_list->if_burn);
		pr_notice("key if_replace=%d\n", key_list->if_replace);
		pr_notice("key if_crypt=%d\n", key_list->if_crypt);
		if (gd->debug_mode >= 7) {
			pr_debug("key data:\n");
			sunxi_dump(key_list->key_data, key_list->len);
		}
		pr_notice("###################\n");
		offset = ((u32)sizeof(sunxi_usb_burn_key_info_t)) +
			 ((key_list->len + 15) & (~15));
		pr_notice("offset=%d\n", offset);
		p_buff += offset;
		...
		if (!key_list->type) {
			...
			strcpy(efuse_key_info.name,
			       (const char *)key_list->name);
			efuse_key_info.len = key_list->len;
			efuse_key_info.key_data = (u8 *)key_list->key_data;

			if (!strcmp(efuse_key_info.name, "rotpk")) {
				pr_notice("ready to burn rootkey\n");
#ifdef CONFIG_SUNXI_KEY_LADDER
				if (sunxi_key_ladder_verify_rotpk_hash(
					    key_list->key_data,
					    key_list->len)) {
					pr_err("verity the rootkey failed, stop burn it\n");
					return -1;
				}
#else
				if (sunxi_verify_rotpk_hash(key_list->key_data,
							    key_list->len)) {
					pr_err("verity the rootkey failed, stop burn it\n");
			...
			} else if (!strncmp(key_list->name, "customer_reserved",
					    (u32)sizeof("customer_reserved"))) {
				u32 sunxi_handle_customer_reserved_id(
					u32 soft_protect_id);
				u32 customer_reserved_id;
				u8 *key_buf = (u8 *)key_list->key_data;
				customer_reserved_id = (key_buf[0] << 8) |
						       key_buf[1];
				customer_reserved_id =
					sunxi_handle_customer_reserved_id(
						customer_reserved_id);
				key_buf[0] = 0;
				key_buf[1] = 0;
			...
			if ((sunxi_get_securemode() ==
			     SUNXI_SECURE_MODE_WITH_SECUREOS) ||
			    (sunxi_probe_secure_monitor())) {
				ret = arm_svc_efuse_write(&efuse_key_info);
				if (ret) {
					pr_err("sunxi_efuse_write failed with:%d\n",
					       ret);
					return -1;
				}
			} else
#endif
			{
				if (sunxi_efuse_usr_write(&efuse_key_info)) {
					return -1;
				}
			}
		}
			...
			if (sunxi_keybox_has_key(key_list->name)) {
				ret = sunxi_keybox_burn_key(
					...
					key_list->if_replace);
				pr_notice("sunxi deal with %s key result:%d\n",
				       key_list->name, ret);
				if (ret) {
					return -1;
				}
...
#endif
			{
				sunxi_secure_object_set(
					key_list->name, key_list->if_crypt,
					key_list->if_replace, 0, 0,
					0); /*set secure storage feature*/

				pr_notice("write key to secure storage\n");
				ret = sunxi_secure_object_write(
					key_list->name,
					(char *)key_list->key_data,
					key_list->len);
				if (ret) {
					pr_err("write key to secure storage failed\n");
					return -1;
				}
			}
		}
...
#ifdef CONFIG_SUNXI_PRIVATE_STORAGE
	if (sunxi_private_storage_exit()) {
		pr_err("sunxi_private_storage_exit err\n");
...
int __sunxi_read_key_by_name(void *buffer, uint buff_len, int *read_len)
{
	...
	if (buffer == NULL) {
		pr_err("the memory input or output is NULL\n");
		return -1;
	}
#ifdef CONFIG_SUNXI_PRIVATE_STORAGE
	if (sunxi_private_storage_init()) {
	...
	*read_len = (u32)sizeof(sunxi_usb_burn_key_info_t);
	...
	if (!key_info->type) {
		...
		if ((gd->securemode == SUNXI_SECURE_MODE_WITH_SECUREOS) ||
		    (sunxi_probe_secure_monitor())) {
			key_data_len = arm_svc_efuse_read(key_info->name,
							  (void *)data_buff);
			if (key_data_len <= 0) {
				pr_err("efuse read %s err\n", key_info->name);
				return -1;
			}
...
#endif
		{
			if (sunxi_efuse_usr_read(key_info->name, (void *)data_buff,
					     &key_data_len)) {
				pr_err("efuse read %s err\n", key_info->name);
				return -1;
			}
		...
		hex2char((void *)(key_info->key_data), (void *)(data_buff),
			 key_data_len);
...
#endif
#ifdef CONFIG_SUNXI_PRIVATE_STORAGE
		pr_notice("read key form securestorage\n");
		ret = sunxi_secure_object_read(
			key_info->name, data_buff,
			(u32)sizeof(sunxi_usb_burn_key_info_t), &key_data_len);
		if (ret < 0) {
			pr_err("read %s form securestorage failed\n",
			       key_info->name);
		} else {
			key_info->len = key_data_len;
			memcpy((void *)(key_info->key_data), (void *)data_buff,
			       key_data_len);
...
int __sunxi_erase_key(void)
{
	...
	if (ret < 0) {
		pr_err("erase secure storage failed\n");
		return -1;
	}

	pr_notice("erase securestorage success\n");
...
int __sunxi_burn_flag(void)
{
	...
	if (sunxi_secure_object_write("key_burned_flag", "key_burned",
				      strlen("key_burned"))) {
		pr_err("save burned flag to securestorage failed\n");
		return -1;
	}

	pr_notice("save burned flag to securestorage success\n");
	sunxi_private_storage_exit();
#endif
	return 0;
}
...
static int sunxi_pburn_init(void)
{
	...
	trans_data.base_recv_buffer =
		(u8 *)memalign(CONFIG_SYS_CACHELINE_SIZE, SUNXI_PBURN_RECV_MEM_SIZE);
	if (!trans_data.base_recv_buffer) {
		pr_err("sunxi usb pburn err: unable to malloc memory for pburn receive\n");

		return -1;
	}

	trans_data.base_send_buffer =
		(u8 *)memalign(CONFIG_SYS_CACHELINE_SIZE, SUNXI_PBURN_SEND_MEM_SIZE);
	if (!trans_data.base_send_buffer) {
		pr_err("sunxi usb pburn err: unable to malloc memory for pburn send\n");
...
************************************************************************************************************
*/
static int sunxi_pburn_standard_req_op(uint cmd, struct usb_device_request *req,
				       uchar *buffer)
{
	...
	default: {
		pr_err("sunxi pburn error: standard req is not supported\n");
...
************************************************************************************************************
*/
static int sunxi_pburn_nonstandard_req_op(uint cmd,
					  ...
					  uchar *buffer, uint data_status)
{
	...
	case 161:
		if (req->bRequest == 0xFE) {
			...
			pr_err("sunxi usb err: unknown ep0 req in pburn\n");
	...
	default:
		pr_err("sunxi usb err: unknown non standard ep0 req\n");
	...
	case 0: //握手
	{
		...
		if (partition_availabe) {
			if (sunxi_partition_get_info("private",
						     &disk_part_info)) {
				pr_err("private partition is not exist\n");
...
#ifdef CONFIG_SUNXI_PRIVATE_STORAGE
		if (sunxi_private_storage_init()) {
			pr_err("init secure storage failed\n");
				   ...
				   struct umass_bbb_csw_t *csw)
{
	...
	default:
		pr_err("usb command = %d not support\n", cbw->CBWCDB[1]);
		break;
	}
}
/*
...
static int sunxi_pburn_state_loop(void *buffer)
{
	...
	case SUNXI_USB_PBURN_IDLE:
		...
		invalidate_dcache_range(
			(unsigned long)sunxi_ubuf,
			(unsigned long)ALIGN(sizeof(sunxi_ubuf_t),
	...
	case SUNXI_USB_PBURN_SETUP:
		invalidate_dcache_range(
			(unsigned long)sunxi_ubuf->rx_req_buffer,
			(unsigned long)ALIGN(sunxi_ubuf->rx_req_length,
					     CONFIG_SYS_CACHELINE_SIZE));

		sunxi_usb_dbg("SUNXI_USB_PBURN_SETUP\n");
		if (sunxi_ubuf->rx_req_length ==
		    (u32)sizeof(struct sunxi_efex_cbw_t)) {
			efex_cbw = (struct sunxi_efex_cbw_t *)
					   sunxi_ubuf->rx_req_buffer;
			if (efex_cbw->magic == CBW_MAGIC) {
				pr_notice("try to update \n");
				...
				sunxi_udc_start_recv_by_dma(
					trans_data.act_recv_buffer,
					16); //start dma to receive data
				break;
			}
		}
		if (sunxi_ubuf->rx_req_length !=
		    (u32)sizeof(struct umass_bbb_cbw_t)) {
			pr_err("sunxi usb error: received bytes 0x%x is not equal cbw struct size 0x%x\n",
		...
		if (CBWSIGNATURE != cbw->dCBWSignature) {
			pr_err("sunxi usb error: the cbw signature 0x%x is bad, need 0x%x\n",
		...
		case 0xf8: /*probe MP tools*/
			if (sunxi_auto_fel_runnning) {
				sunxi_auto_fel_process(cbw, &csw);
			} else {
				/*ignore*/
			}
			break;
		case 0xf0: //burn user data
			...
			case 0: //握手
			{
				...
				sunxi_usb_pburn_status =
					SUNXI_USB_PBURN_SEND_DATA;

				trans_data.act_send_buffer =
					trans_data.base_send_buffer;
				trans_data.send_size =
					min(cbw->dCBWDataTransferLength,
					    (u32)sizeof(__usb_handshake_sec_t));

				sunxi_usb_burn_from_boot_setup = 1;

				if (private_data_ext_buff == NULL) {
					pr_notice("malloc memory\n");
					private_data_ext_buff =
						(u8 *)memalign(CONFIG_SYS_CACHELINE_SIZE, DATA_BUFFER_MAX_SIZE);
					if (private_data_ext_buff == NULL) {
						pr_err("there is no memory to store all user key data\n");
						csw.bCSWStatus = -1;
					} else {
						csw.bCSWStatus = 0;
						sunxi_usb_burn_from_boot_handshake =
							1;
					}
				} else {
					csw.bCSWStatus = 0;
			...
			case 1: //小机端接收数据
			{
				...
				memset(private_data_ext_buff, 0,
				...
				sunxi_usb_burn_main_info_t *key_main =
					(sunxi_usb_burn_main_info_t *)private_data_ext_buff;
				sunxi_usb_burn_key_info_t *key_list =
					(sunxi_usb_burn_key_info_t *)&key_main->key_info;

				pr_notice("recv_size=%d\n", trans_data.recv_size);
				/* If it is a security key, printing is not allowed! */
				if (!sunxi_keybox_has_key(key_list->name))
					sunxi_dump(private_data_ext_buff,
						   trans_data.recv_size);

				sunxi_usb_pburn_status =
					SUNXI_USB_PBURN_RECEIVE_NULL;
			} break;
			...
			case 2: //工具端声明数据传输已经完毕，要求获取烧录状态
			{
				...
				int ret =
					__sunxi_burn_key(private_data_ext_buff,
							 trans_data.recv_size);
			...
			case 4: //关闭usb
			{
				...
				sunxi_udc_send_data(
					(void *)trans_data.act_send_buffer,
					trans_data.send_size);
			...
			case 6: //huk
			{
				...
				trans_data.act_send_buffer =
					trans_data.base_send_buffer;
				trans_data.send_size = min(
					cbw->dCBWDataTransferLength, (u32)32);

				if (ret < 0) {
					pr_err("smc_create_huk failed\n");
					csw.bCSWStatus = -1;
				} else {
					if (!ret)
						pr_notice("smc_create_huk successed\n");
					else
						pr_err("smc_create_huk already exist\n");
					memcpy((void *)trans_data
						       .act_send_buffer,
					       huk, 32);
				}
			...
			case 7: //read key one by one
			{
				...
				ret = __sunxi_read_key_by_name(
					private_data_ext_buff,
					DATA_BUFFER_MAX_SIZE, &read_len);
				if (!ret) {
					pr_notice("read key success\n");
					csw.bCSWStatus = 0;
				} else {
					pr_err("read key failed\n");
					csw.bCSWStatus = 1;
				}
				...
				trans_data.act_send_buffer =
					trans_data.base_send_buffer;
				trans_data.send_size =
					max(cbw->dCBWDataTransferLength,
					    (u32)read_len);

				memcpy((void *)trans_data.act_send_buffer,
				       (void *)private_data_ext_buff, read_len);
				pr_notice("read_len=%d\n", read_len);
				pr_notice("the send data:\n");
				sunxi_dump((void *)private_data_ext_buff,
					   read_len);
			} break;
			...
			case 8: //erase all key
			{
				...
				if (ret < 0) {
					pr_err("sunxi erase key failed\n");
					strcpy(handshake->magic,
					       "usb_erase_failed");
					handshake->err_no =
						ERR_NO_ERASE_KEY_FAILED;
					break;
				}

				strcpy(handshake->magic, "usb_erase_success");
				handshake->err_no = ERR_NO_SUCCESS;
				pr_notice("sunxi erase all key success\n");
			} break;
			...
			case USB_BURN_ANDROID_RECOVERY: {
				...
				if (ret < 0) {
					pr_err("usb_burn_androidrecovery failed\n");
					csw.bCSWStatus = -1;
				} else {
					pr_notice("usb_burn_androidrecovery successed\n");
					csw.bCSWStatus = 0;
				}
				sunxi_usb_pburn_status =
					SUNXI_USB_PBURN_SEND_DATA;
	...
	case SUNXI_USB_PBURN_SEND_DATA:
		...
		sunxi_usb_pburn_status = SUNXI_USB_PBURN_STATUS;
		flush_dcache_range((unsigned long)trans_data.act_send_buffer,
				   (unsigned long)trans_data.act_send_buffer +
					   ALIGN(trans_data.send_size,
						 CONFIG_SYS_CACHELINE_SIZE));
		pr_notice("SUNXI_USB_SEND_DATA=%d\n", trans_data.send_size);
		sunxi_udc_send_data((void *)trans_data.act_send_buffer,
				    trans_data.send_size);
#if defined(SUNXI_USB_30)
	...
	case SUNXI_USB_PBURN_RECEIVE_DATA:
		...
		if (sunxi_usb_pburn_write_enable == 1) {
			invalidate_dcache_range(
				(unsigned long)trans_data.act_recv_buffer,
				(unsigned long)trans_data.act_recv_buffer +
					ALIGN(trans_data.recv_size,
					      CONFIG_SYS_CACHELINE_SIZE));
			sunxi_usb_dbg("write flash, start 0x%x, sectors 0x%x\n",
				      pburn_flash_start,
				      trans_data.recv_size / 512);
			ret = sunxi_flash_write(
				pburn_flash_start,
				(trans_data.recv_size + 511) / 512,
				(void *)trans_data.act_recv_buffer);
			if (!ret) {
				pr_err("sunxi flash write err: start,0x%x sectors 0x%x\n",
				       pburn_flash_start,
				       (trans_data.recv_size + 511) / 512);
	...
	case SUNXI_USB_PBURN_STATUS:
...
#endif
		{
			sunxi_usb_pburn_status = SUNXI_USB_PBURN_IDLE;
			sunxi_ubuf->rx_ready_for_data = 0;
			__sunxi_pburn_send_status(
				&csw, (u32)sizeof(struct umass_bbb_csw_t));
		}
	...
	case SUNXI_USB_PBURN_EXIT:
		...
		pr_notice("SUNXI_USB_PBURN_EXIT\n");

		sunxi_usb_pburn_status = SUNXI_USB_PBURN_IDLE;
		sunxi_ubuf->rx_ready_for_data = 0;
		__sunxi_pburn_send_status(&csw,
					  (u32)sizeof(struct umass_bbb_csw_t));

		pr_notice("Device will shutdown in 3 Secends...\n");
		__msdelay(3000);
	...
	case SUNXI_USB_PBURN_RECEIVE_NULL:
		...
		if (sunxi_usb_pburn_write_enable == 1) {
			...
			sunxi_usb_pburn_status = SUNXI_USB_PBURN_STATUS;
			invalidate_dcache_range(
				(unsigned long)private_data_ext_buff,
				(unsigned long)private_data_ext_buff +
					ALIGN(DATA_BUFFER_MAX_SIZE,
					      CONFIG_SYS_CACHELINE_SIZE));
		}
		break;
...
sunxi_usb_module_init(SUNXI_USB_DEVICE_BURN, sunxi_pburn_init, sunxi_pburn_exit,
		      sunxi_pburn_reset, sunxi_pburn_standard_req_op,
		      sunxi_pburn_nonstandard_req_op, sunxi_pburn_state_loop,
		      sunxi_pburn_usb_rx_dma_isr, sunxi_pburn_usb_tx_dma_isr);
