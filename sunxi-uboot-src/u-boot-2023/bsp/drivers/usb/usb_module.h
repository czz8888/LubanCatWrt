...
#ifdef CONFIG_AW_EFEX
#define SUNXI_USB_DEVICE_EFEX 2
#endif

#ifdef CONFIG_AW_FASTBOOT
#define SUNXI_USB_DEVICE_FASTBOOT 3
#endif

#ifdef CONFIG_AW_BURN
...
typedef struct sunxi_usb_setup_req_s {
	...
	int (*standard_req_op)(uint cmd, struct usb_device_request *req,
			       uchar *buffer);
	int (*nonstandard_req_op)(uint cmd, struct usb_device_request *req,
...
#define __sunxi_usb_module_init(name, state_init, state_exit, state_reset,    \
...
#define sunxi_usb_module_init(name, state_init, state_exit, state_reset,       \
			      standard_req_op, nonstandard_req_op, state_loop, \
			      dma_rx_isr, dma_tx_isr)                          \
	__sunxi_usb_module_init(name, state_init, state_exit, state_reset,     \
				standard_req_op, nonstandard_req_op,           \
				state_loop, dma_rx_isr, dma_tx_isr)

#define __sunxi_usb_module_reg(name) (sunxi_udev_active = &setup_req_##name)
...
#define sunxi_usb_module_ext(name) __sunxi_usb_module_ext(name)
