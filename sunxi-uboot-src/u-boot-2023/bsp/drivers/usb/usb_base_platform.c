...
static int sunxi_usb_port = SUNXI_USB_USBC0; /* 默认使用USB0*/
...
static ulong usb_dma_trans_mem_buf;
...
/* for usb config */
__weak void sunxi_usb_otg_phy_config(void)
{
	uint reg_val = 0;
	pr_info("weak:sunxi_usb_otg_phy_config\n");
...
/* for usb config */
void *get_sunxi_ubuf(void)
{
	return (void *)&sunxi_ubuf;
}

/* check burn firmware by usb2.0(USB0) or usb3.1(USB2_U2) */
int usb_is_dwc3(void)
{
	return sunxi_usb_port;
}
...
void sunxi_usb_irq(void *data)
{
	...
	if (misc_irq & USBC_INTUSB_SOF) {
		...
		USBC_INT_DisableUsbMiscUint(sunxi_udc_source.usbc_hd,
					    USBC_INTUSB_SOF);
	...
	if (tx_irq & (1 << SUNXI_USB_CTRL_EP_INDEX)) {
		...
		USBC_INT_ClearEpPending(sunxi_udc_source.usbc_hd,
					USBC_EP_TYPE_TX,
...
int sunxi_usb_init(int delaytime)
{
	...
	sunxi_udc_source.bulk_out_addr =
		sunxi_udc_source.bulk_in_addr + sunxi_udc_source.fifo_size * 2;
	//内存资源
	memset(&sunxi_ubuf, 0, sizeof(sunxi_ubuf_t));

	sunxi_ubuf.rx_base_buffer =
		(uchar *)memalign(CONFIG_SYS_CACHELINE_SIZE, 1024);
	...
	USBC_Dev_ConfigTransferMode(sunxi_udc_source.usbc_hd, USBC_TS_TYPE_BULK,
				    USBC_TS_MODE_FS);
#else
	USBC_Dev_ConfigTransferMode(sunxi_udc_source.usbc_hd, USBC_TS_TYPE_BULK,
				    USBC_TS_MODE_HS);
#endif
	//配置发送端dma资源
	usb_dma_setting(sunxi_udc_source.dma_send_channal,
			USB_DMA_FROM_DRAM_TO_HOST, SUNXI_USB_BULK_IN_EP_INDEX);
	usb_dma_set_pktlen(sunxi_udc_source.dma_send_channal,
			   HIGH_SPEED_EP_MAX_PACKET_SIZE);
	//配置接收端dma资源
	usb_dma_setting(sunxi_udc_source.dma_recv_channal,
			USB_DMA_FROM_HOST_TO_DRAM, SUNXI_USB_BULK_OUT_EP_INDEX);
	usb_dma_set_pktlen(sunxi_udc_source.dma_recv_channal,
			   HIGH_SPEED_EP_MAX_PACKET_SIZE);
	...
	USBC_INT_EnableUsbMiscUint(sunxi_udc_source.usbc_hd,
				   USBC_INTUSB_SUSPEND | USBC_INTUSB_RESUME |
					   USBC_INTUSB_RESET | USBC_INTUSB_SOF);

	/* enbale ep0_tx_irq */
	USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX,
			  SUNXI_USB_CTRL_EP_INDEX);
...
static void __usb_recv_by_dma_isr(void *p_arg)
{
	...
	old_ep_idx = USBC_GetActiveEp(sunxi_udc_source.usbc_hd);
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd,
			    SUNXI_USB_BULK_OUT_EP_INDEX); //选择RXEP

	//选择使用IO方式搬运数据
	sunxi_usb_dbg("select io mode to transfer data\n");
	USBC_Dev_ClearEpDma(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX);

	invalidate_dcache_range(usb_dma_trans_mem_buf, usb_dma_trans_mem_buf + sunxi_ubuf.request_size);

	if (usb_dma_trans_unaliged_bytes) {
		uint fifo, this_len;

		this_len = USBC_ReadLenFromFifo(sunxi_udc_source.usbc_hd,
						USBC_EP_TYPE_RX);
		fifo = USBC_SelectFIFO(sunxi_udc_source.usbc_hd,
				       SUNXI_USB_BULK_OUT_EP_INDEX);
		USBC_ReadPacket(sunxi_udc_source.usbc_hd, fifo, this_len,
				usb_dma_trans_unaligned_buf);
		__usb_readcomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX,
				   1); //返回状态
		usb_dma_trans_unaliged_bytes = 0;
	}
	//如果当前dma传输的不是完整包，则需要手动清除中断
	if (sunxi_ubuf.request_size % sunxi_udc_source.bulk_ep_max) {
		USBC_Dev_ReadDataStatus(sunxi_udc_source.usbc_hd,
					USBC_EP_TYPE_RX, 1);
...
static void __usb_readcomplete(__hdle hUSB, u32 ep_type, u32 complete)
{
	...
	if (ep_type == USBC_EP_TYPE_EP0) {
		...
		/* clear irq */
		USBC_INT_ClearEpPending(hUSB, USBC_EP_TYPE_TX,
					SUNXI_USB_CTRL_EP_INDEX);
	}
...
static void __usb_bulk_ep_reset(void)
{
	...
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd,
			    SUNXI_USB_BULK_IN_EP_INDEX);
	USBC_Dev_ConfigEp(sunxi_udc_source.usbc_hd, USBC_TS_TYPE_BULK,
			  USBC_EP_TYPE_TX, 1,
			  sunxi_udc_source.bulk_ep_max & 0x7ff);
	USBC_ConfigFifo(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, 1,
			sunxi_udc_source.fifo_size,
			(uint)sunxi_udc_source.bulk_out_addr);
	USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX,
			  SUNXI_USB_BULK_IN_EP_INDEX);
	/* rx */
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd,
	...
	USBC_Dev_ConfigEp(sunxi_udc_source.usbc_hd, USBC_TS_TYPE_BULK,
			  USBC_EP_TYPE_RX, 1,
			  sunxi_udc_source.bulk_ep_max & 0x7ff);
	USBC_ConfigFifo(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX, 1,
			sunxi_udc_source.fifo_size,
			(uint)sunxi_udc_source.bulk_in_addr);
	USBC_INT_EnableEp(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX,
			  SUNXI_USB_BULK_OUT_EP_INDEX);
...
static int __usb_read_ep0_data(void *buffer, uint data_type)
{
	...
	fifo = USBC_SelectFIFO(sunxi_udc_source.usbc_hd,
			       SUNXI_USB_CTRL_EP_INDEX);
	fifo_count = USBC_ReadLenFromFifo(sunxi_udc_source.usbc_hd,
					  USBC_EP_TYPE_EP0);
	if (!data_type) {
		if (fifo_count != 8) {
	...
	USBC_ReadPacket(sunxi_udc_source.usbc_hd, fifo, fifo_count,
			(void *)buffer);
...
static int __sunxi_udc_set_address(uchar address)
{
	...
	if (USBC_Dev_QueryTransferMode(sunxi_udc_source.usbc_hd) ==
	    USBC_TS_MODE_HS) {
		sunxi_udc_source.speed = USB_SPEED_HIGH;
...
static int __usb_write_fifo(uchar *buffer, unsigned int buffer_size)
{
	...
	while (left) {
		...
		this_len = USBC_WritePacket(sunxi_udc_source.usbc_hd, fifo,
					    this_len, buffer + transfered);

		transfered += this_len;
		left -= this_len;

		__usb_writecomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX,
				    1);
	}
...
int sunxi_udc_start_recv_by_dma(void *mem_base, uint length)
{
	...
	usb_dma_trans_mem_buf = mem_buf;

	old_ep_idx = USBC_GetActiveEp(sunxi_udc_source.usbc_hd);
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd,
			    SUNXI_USB_BULK_OUT_EP_INDEX); //选择当前EP
	//usb控制器选择dma传输方式
	USBC_Dev_ConfigEpDma(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX);

	//刷掉cache
	flush_cache(mem_buf, ALIGN(length, CONFIG_SYS_CACHELINE_SIZE));
	//使能dma传输
...
void sunxi_udc_send_setup(uint bLength, void *buffer)
{
	...
	if (!bLength) {
		/* sent zero packet */
		__usb_writecomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0,
				    1);
	} else {
		fifo = USBC_SelectFIFO(sunxi_udc_source.usbc_hd,
				       SUNXI_USB_CTRL_EP_INDEX);
		USBC_WritePacket(sunxi_udc_source.usbc_hd, fifo, bLength,
				 (void *)buffer);
		__usb_writecomplete(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0,
				    1);
	}
...
static int ep0_recv_op(void)
{
	u32 old_ep_index = 0;
	int ret = 0;
	static uint ep0_stage;

	if (!ep0_stage) {
		memset(&sunxi_udc_source.standard_reg, 0,
	...
	if (USBC_Dev_IsEpStall(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_EP0)) {
		...
		USBC_Dev_EpClearStall(sunxi_udc_source.usbc_hd,
				      USBC_EP_TYPE_EP0);
		ret = -1;
	...
	//检查读ep0数据是否完成
	if (USBC_Dev_IsReadDataReady(sunxi_udc_source.usbc_hd,
				     USBC_EP_TYPE_EP0)) {
		uint status;

		if (!ep0_stage) {
			status = __usb_read_ep0_data(
				&sunxi_udc_source.standard_reg, ep0_stage);
		} else {
			status = __usb_read_ep0_data(sunxi_usb_ep0_buffer,
						     ep0_stage);
		}
		if (status != 0) {
		...
		if (sunxi_udc_source.address) {
			__sunxi_udc_set_address(sunxi_udc_source.address &
						0xff);
			pr_info("set address 0x%x ok\n",
	...
	if (USB_REQ_TYPE_STANDARD ==
	    (sunxi_udc_source.standard_reg.bmRequestType & USB_REQ_TYPE_MASK)) {
		...
		case USB_REQ_CLEAR_FEATURE: //   0x01
		{
			...
			if (USB_DIR_OUT ==
				...
				ret = sunxi_udev_active->standard_req_op(
					...
					&sunxi_udc_source.standard_reg, NULL);
			}
		...
		ret = sunxi_udev_active->nonstandard_req_op(
			USB_REQ_GET_STATUS, &sunxi_udc_source.standard_reg,
			sunxi_usb_ep0_buffer, ep0_stage);
...
static int eprx_recv_op(void)
{
	...
	if (USBC_Dev_IsEpStall(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX)) {
		...
		if (USBC_Dev_IsReadDataReady(sunxi_udc_source.usbc_hd,
					     USBC_EP_TYPE_RX)) {
			this_len = USBC_ReadLenFromFifo(
				sunxi_udc_source.usbc_hd, USBC_EP_TYPE_RX);
			if (get_fastboot_data_flag() == 1) {
				fifo = USBC_SelectFIFO(
					sunxi_udc_source.usbc_hd,
					SUNXI_USB_BULK_OUT_EP_INDEX);

				sunxi_ubuf.rx_req_length = USBC_ReadPacket(
					sunxi_udc_source.usbc_hd, fifo,
					this_len, sunxi_ubuf.rx_req_buffer);
				sunxi_ubuf.rx_req_buffer += this_len;

				sunxi_usb_dbg("special read ep bytes 0x%x\n",
					      sunxi_ubuf.rx_req_length);
				__usb_readcomplete(sunxi_udc_source.usbc_hd,
						   USBC_EP_TYPE_RX,
						   1); //返回状态
				...
				sunxi_usb_dbg(
					"eprx do nothing and left it to dma\n");
			}
		} else {
			sunxi_usb_dbg("sunxi usb rxdata not ready\n");
		}
...
int sunxi_usb_main_loop(uint dev_name, int delaytime)
{
	...
	if (sunxi_usb_dev_register(dev_name) < 0) {
