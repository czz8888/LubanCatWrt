...
__u32 usbc_new_phyx_tp_read(__u32 regs, __u32 addr, __u32 len)
{
	int temp, i, j, ret = 0;

	temp = USBC_Readb(regs + USBC_REG_o_PHYCTL);
	temp |= (0x1 << 1);
	USBC_Writeb(temp, regs + USBC_REG_o_PHYCTL);

	for (j = len; j > 0; j--) {
		USBC_Writeb((addr + j - 1), regs + USBC_REG_o_PHYCTL + 1);
...
int usbc_new_phyx_tp_write(__u32 regs, int addr, int data, int len)
{
	int temp, j, dtmp = 0;
	...
	for (j = 0; j < len; j++) {
		...
		USBC_Writeb(addr + j, regs + USBC_REG_o_PHYCTL + 1);
...
static __u32 usb_phy0_write(__u32 addr, __u32 data, __u32 dmask,
			    ulong usbc_base_addr)
{
	__u32 i = 0;
