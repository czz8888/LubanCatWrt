			     ...
			     __u32 ep_type)
{
	...
	reg_val = readl((const volatile void __iomem *)(usbc_otg->base_addr +
							USBC_REG_o_DMA_CONFIG +
							0x10 * dma_chan));
	reg_val &= ~((1 << 4) | (0xf << 0));
	reg_val |= ((trans_dir & 1) << 4);
	reg_val |= ((ep_type & 0xf) << 0);
	writel(reg_val, (volatile void __iomem *)(usbc_otg->base_addr +
						  USBC_REG_o_DMA_CONFIG +
						  0x10 * dma_chan));
...
int USBC_Dma_Start(__hdle hUSB, __u32 dma_chan, __u32 addr, __u32 pkt_len)
{
	...
	writel(addr, (volatile void __iomem *)(usbc_otg->base_addr +
					       USBC_REG_o_DMA_ADDR +
					       0x10 * dma_chan));
	...
	writel(pkt_len, (volatile void __iomem *)(usbc_otg->base_addr +
						  USBC_REG_o_DMA_SIZE +
						  0x10 * dma_chan));
...
int USBC_Dma_Int_Query(__hdle hUSB)
{
	...
	return readl((const volatile void __iomem *)(usbc_otg->base_addr +
...
int USBC_Dma_Int_Clear(__hdle hUSB)
{
	...
	writel(0xff, (volatile void __iomem *)(usbc_otg->base_addr +
					       USBC_REG_o_DMA_STATUS));
