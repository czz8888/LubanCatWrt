...
static __u32 usbc_base_address[USBC_MAX_CTL_NUM] = {
	SUNXI_USBOTG_BASE
}; /* usb base address */
static __usbc_otg_t
	usbc_otg_array[USBC_MAX_OPEN_NUM]; /* usbc 内部使用, 用来管理USB端口 */
...
{
	__usbc_otg_t *usbc_otg = (__usbc_otg_t *)(ulong)hUSB;
	__u8 reg_val = 0;
...
__u32 USBC_WritePacket(__hdle hUSB, __u32 fifo, __u32 cnt, void *buff)
{
	...
	int i;
	printf("usb fifo:0x%08x, cnt:%d\n", fifo, cnt);
	buf8 = (__u8 *)buff;
	printf("usb fifo buf tx :\n");
	for (i = 0; i < cnt; i++)
...
#ifdef SUNXI_USB_DEBUG
	int i;
	printf("usb fifo:0x%08x, cnt:%d, buff:0x%08x\n", fifo, cnt, (uint)buff);
	buf8 = (__u8 *)buff;
	printf("usb fifo buf rx:\n");
	for (i = 0; i < cnt; i++)
		printf("0x%02x ", buf8[i]);
	printf("\n");
#endif
...
***********************************************************************************
*/
void __USBC_ConfigFifo_TxEp(ulong usbc_base_addr, __u32 is_double_fifo,
...
***********************************************************************************
*/
void __USBC_ConfigFifo_RxEp(ulong usbc_base_addr, __u32 is_double_fifo,
			    __u32 fifo_size, __u32 fifo_addr)
{
	__u32 temp = 0;
	__u32 size = 0; //fifo_size = (size + 3)的2次方
		     ...
		     __u32 fifo_size, __u32 fifo_addr)
{
	...
	case USBC_EP_TYPE_TX:
		__USBC_ConfigFifo_TxEp(usbc_otg->base_addr, is_double_fifo,
				       fifo_size, fifo_addr);
		break;

	case USBC_EP_TYPE_RX:
		__USBC_ConfigFifo_RxEp(usbc_otg->base_addr, is_double_fifo,
				       fifo_size, fifo_addr);
		break;
...
*******************************************************************************
*/
static void USBC_PrintEPReg(ulong usbc_base, __s32 ep_index, char *str,
			    USBC_PRINT print)
{
	...
	print("TXFADDRx(%d)             = 0x%x\n", ep_index,
	      USBC_Readb(usbc_base + USBC_REG_o_TXFADDRx));
	print("TXHADDRx(%d              = 0x%x\n", ep_index,
	      USBC_Readb(usbc_base + USBC_REG_o_TXHADDRx));
	print("TXHPORTx(%d)             = 0x%x\n", ep_index,
	      USBC_Readb(usbc_base + USBC_REG_o_TXHPORTx));
	print("RXFADDRx(%d)             = 0x%x\n", ep_index,
	      USBC_Readb(usbc_base + USBC_REG_o_RXFADDRx));
	print("RXHADDRx(%d)             = 0x%x\n", ep_index,
	      USBC_Readb(usbc_base + USBC_REG_o_RXHADDRx));
	print("RXHPORTx(%d)             = 0x%x\n", ep_index,
	      USBC_Readb(usbc_base + USBC_REG_o_RXHPORTx));
	print("RPCOUNTx(%d)             = 0x%x\n", ep_index,
	      (u32)USBC_Readw(usbc_base + USBC_REG_o_RPCOUNT));
...
__hdle USBC_open_otg(__u32 otg_no)
{
	...
	return (__hdle)(ulong)(&(usbc_otg[otg_no]));
}
