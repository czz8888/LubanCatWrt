...
{
	USBC_REG_set_bit_w(USBC_BP_CSR0_H_DisPing,
			   USBC_REG_CSR0(usbc_base_addr));
}
...
static void __USBC_Host_ep0_ConfigEp(ulong usbc_base_addr, __u32 ts_mode,
				     __u32 interval)
{
	//--<1>--config ep0 csr
	USBC_Writew(1 << USBC_BP_CSR0_H_FlushFIFO,
...
static void __USBC_Host_Tx_EpType(ulong usbc_base_addr, __u32 ep_index,
				  __u32 ts_mode, __u32 ts_type)
{
	__u32 reg_val = 0;
...
static void __USBC_Host_Tx_ConfigEp(ulong usbc_base_addr, __u32 ep_index,
				    __u32 ts_mode, __u32 ts_type,
...
static void __USBC_Host_Rx_EpType(ulong usbc_base_addr, __u32 ep_index,
				  __u32 ts_mode, __u32 ts_type)
{
	__u32 reg_val = 0;
...
static void __USBC_Host_Rx_ConfigEp(ulong usbc_base_addr, __u32 ep_index,
				    __u32 ts_mode, __u32 ts_type,
				    __u32 is_double_fifo, __u32 ep_MaxPkt,
				    __u32 interval)
{
					  ...
					  __u32 ep_index)
{
	...
	case USBC_EP_TYPE_TX:
		USBC_Writeb(0x00,
			    USBC_REG_TXFADDRx(usbc_otg->base_addr, ep_index));
		USBC_Writeb(0x00,
	...
	case USBC_EP_TYPE_RX:
		USBC_Writeb(0x00,
			    USBC_REG_RXFADDRx(usbc_otg->base_addr, ep_index));
		USBC_Writeb(0x00,
			...
			__u32 ep_MaxPkt, __u32 interval)
{
	...
	switch (ep_type) {
	case USBC_EP_TYPE_EP0:
		__USBC_Host_ep0_ConfigEp(usbc_otg->base_addr, ts_mode,
					 interval);
		break;

	case USBC_EP_TYPE_TX:
		__USBC_Host_Tx_ConfigEp(usbc_otg->base_addr, ep_index, ts_mode,
					ts_type, is_double_fifo, ep_MaxPkt,
					interval);
		break;

	case USBC_EP_TYPE_RX:
		__USBC_Host_Rx_ConfigEp(usbc_otg->base_addr, ep_index, ts_mode,
					ts_type, is_double_fifo, ep_MaxPkt,
					interval);
		break;
...
__s32 USBC_Host_WriteDataStatus(__hdle hUSB, __u32 ep_type, __u32 complete)
{
	...
	if (complete) {
		return __USBC_Host_WriteDataComplete(usbc_otg->base_addr,
						     ep_type);
	} else {
