...
static void __USBC_Dev_Tx_ConfigEp(ulong usbc_base_addr, __u32 ts_type,
...
static void __USBC_Dev_Rx_ConfigEp(ulong usbc_base_addr, __u32 ts_type,
				   __u32 is_double_fifo, __u32 ep_MaxPkt)
{
	__u16 reg_val = 0;
	__u16 temp = 0;
...
static void __USBC_Dev_ClearDma_Trans(ulong usbc_base_addr)
{
	...
	reg_val = readl((const volatile void __iomem *)(usbc_base_addr +
							USBC_REG_o_PCTL));
	reg_val &= ~(1 << 24);
	writel(reg_val,
	       ((volatile void __iomem *)(usbc_base_addr + USBC_REG_o_PCTL)));
}
			...
			__u32 is_double_fifo, __u32 ep_MaxPkt)
{
	...
	case USBC_EP_TYPE_TX:
		__USBC_Dev_Tx_ConfigEp(usbc_otg->base_addr, ts_type,
				       is_double_fifo, ep_MaxPkt);
		break;

	case USBC_EP_TYPE_RX:
		__USBC_Dev_Rx_ConfigEp(usbc_otg->base_addr, ts_type,
				       is_double_fifo, ep_MaxPkt);
		break;
...
__s32 USBC_Dev_WriteDataStatus(__hdle hUSB, __u32 ep_type, __u32 complete)
{
	...
	if (complete) {
		return __USBC_Dev_WriteDataComplete(usbc_otg->base_addr,
						    ep_type);
	} else {
...
__s32 USBC_Dev_ReadDataStatus(__hdle hUSB, __u32 ep_type, __u32 complete)
{
	...
	if (complete) {
		return __USBC_Dev_ReadDataComplete(usbc_otg->base_addr,
						   ep_type);
	} else {
