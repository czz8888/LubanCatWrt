...
#else
	pr_err("You need to enable CONFIG_CMD_FAT to update disp_reserve\n");
#endif
}
...
int save_bmp_logo_to_kernel(void)
{
	...
	if (node < 0) {
		pr_err("%s:disp_fdt_nodeoffset %s fail\n", __func__, "disp");
		goto exit;
	}

	ret = fdt_setprop_u32(working_fdt, node, name, (uint32_t)value);
	if (ret < 0)
		pr_err("fdt_setprop_u32 %s.%s(0x%x) fail.err code:%s\n",
...
int fat_read_logo_to_kernel(char *name)
{
	...
	char part_info[16] = {0};
	int partno = -1;
	struct bmp_header *bmp_head_addr = (struct bmp_header *)CFG_SYS_SDRAM_BASE;
	char *bmp_buff = NULL;

	if (bmp_head_addr) {
		sprintf(bmp_head, "%lx", (ulong)bmp_head_addr);
	} else {
		pr_err("sunxi bmp: alloc buffer for %s fail\n", name);
		return -1;
	}
	...
	if (partno < 0) {
		pr_err("Get boot-resource partition number fail!\n");
		return -1;
	}
	...
	if (disp_fat_load(0, 0, 5, argv)) {
		pr_err("sunxi bmp info error : unable to open logo file %s\n",
		       argv[4]);
		return -1;
	}
	if (bmp_head_addr->signature[0] != 'B' ||
	    bmp_head_addr->signature[1] != 'M') {
		pr_err("This not a BMP file!\n");
		return 0;
	}
...
int read_bmp_to_kernel(char *partition_name)
{
	...
	if (NULL == bmp_buff) {
	    pr_err("bmp buffer malloc error!\n");
	    return -1;
	}
