...
static int do_sunxi_bmp_info(struct cmd_tbl *cmdtp, int flag, int argc,
			     char *const argv[])
{
	uint addr;
	int ret = 0;
	...
	if (argc == 2) {
		...
		if (sunxi_flash_get_storage() == STORAGE_EMMC)
			bmp_argv = {"fatload", "mmc", part_info, load_addr, filename, NULL};
		else
			bmp_argv = {"fatload", "sunxiflash", part_info, load_addr, filename, NULL};
...
static int do_sunxi_bmp_display(struct cmd_tbl *cmdtp, int flag, int argc,
				char *const argv[])
{
	unsigned long addr;
...
int sunxi_bmp_display(char *name)
{
	...
	/* By default, the startup logo will be displayed in */
	/* the framebuffer of each screen. */
...
int do_sunxi_logo(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
