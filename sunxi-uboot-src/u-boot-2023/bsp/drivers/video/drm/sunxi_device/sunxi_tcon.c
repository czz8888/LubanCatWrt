...
static int sunxi_tcon_lcd_set_clk(struct sunxi_tcon *hwtcon, unsigned long pixel_clk)
{
	...
	struct reset_ctl *rsts[] = {hwtcon->rst_bus_tcon};
		...
		unsigned long pclk)
{
	...
	if (hwtcon->rst_bus_tcon) {
		ret = reset_deassert(hwtcon->rst_bus_tcon);
		if (ret != 0) {
...
static int _sunxi_tcon_hdmi_clk_unset(struct sunxi_tcon *hwtcon)
{
	...
	reset_assert(hwtcon->rst_bus_tcon);
...
static int edp_tcon_clk_disable(struct sunxi_tcon *hwtcon)
{
	...
	ret = reset_assert(hwtcon->rst_bus_tcon);
	if (ret) {
...
static int sunxi_tcon_parse_dts(struct udevice *dev)
{
	...
	hwtcon->rst_bus_tcon = devm_reset_control_get(dev, "rst_bus_tcon");
	if (IS_ERR_OR_NULL(hwtcon->rst_bus_tcon)) {
		DRM_INFO("fail to get reset clk for tcon\n");
	}
