...
struct tcon_top {
	...
	struct reset_ctl *rst_bus_dpss;
	struct reset_ctl *rst_bus_reg;
...
static int sunxi_tcon_top_probe(struct udevice *dev)
{
	...
	top->rst_bus_dpss =
		devm_reset_control_get(dev, "rst_bus_dpss_top");
	if (IS_ERR(top->rst_bus_dpss)) {
		DRM_INFO("fail to get reset rst_bus_dpss_top\n");
	}

	top->rst_bus_reg =
		devm_reset_control_get(dev, "rst_bus_reg");

	if (IS_ERR(top->rst_bus_reg)) {
		DRM_INFO("fail to get reset rst_bus_reg\n");
	}
...
U_BOOT_DRIVER(tcon_top) = {
	...
	.priv_auto = sizeof(struct tcon_top),
};
...
int sunxi_tcon_top_clk_enable(struct udevice *tcon_top)
{
	...
	struct reset_ctl *rsts[] = {topif->rst_bus_dpss, topif->rst_bus_reg};
