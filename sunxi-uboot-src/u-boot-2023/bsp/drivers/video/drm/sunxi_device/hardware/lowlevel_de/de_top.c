...
#define DE_RESERVE_CTRL_OFFSET			(0x800C)
...
#define DE_RTWB_CTL_OFFSET_V21X			(0x20)
#define DE_RTWB_MUX_OFFSET_V21X			(0x24)
...
static int sunxi_get_soc_ver(void)
{
	...
	uclass_get_device_by_driver(UCLASS_MISC,
				    DM_DRIVER_GET(sunxi_efuse), &efuse_dev);
	if (!efuse_dev) {
		DRM_ERROR("Get sunxi efuse dev fail!\n");
		return 0;
	}

	return misc_ioctl(efuse_dev, SUNXI_EFUSE_IOCTL_GET_SOC_VER, NULL);
}
...
static struct de_top_desc de350 = {
	...
	.rtwb_ctl_offset = DE_RTWB_CTL_OFFSET,
...
static struct de_top_desc de210 = {
	...
	.rtwb_ctl_offset = DE_RTWB_CTL_OFFSET_V21X,
...
static struct de_top_desc de212 = {
	.version = 0x212,
	.support_rcq_gate = 1,
	//.auto_clk_offset = RTMX_AUTO_CLK_OFFSET_V21X,
	.de_reset_offset = DE_RESET_OFFSET_V21X,
	...
	.rcq_ctl_offset = RTMX_RCQ_CTL_OFFSET_V21X,
	.rcq_header_laddr_offset = RTMX_RCQ_HEADER_LADDR_OFFSET_V21X,
	/* from de210. maybe change. */
	.query_state_with_clear = de_top_query_state_with_clear_v21x,
	.display_config = de_top_display_config_v2,
	.check_finish = de_top_check_display_rcq_update_finish_with_clear,

//	.busy_offset = RTMX_GLB_STS_OFFSET_V21X,
	.rtwb_mux_offset = DE_RTWB_MUX_OFFSET_V21X,
	.rtwb_ctl_offset = DE_RTWB_CTL_OFFSET_V21X,
	.rtwb_timing_shift = 4,
};

static struct de_top_desc *de_version[] = {
	&de350, &de352, &de355, &de210, &de201, &de212,
};
...
static inline void __de_mbus_clk_enable(struct de_top_handle *hdl, bool en)
{
	...
	reg_val = DE_SET_BITS(0, 1, reg_val, en ? 1 : 0);
...
static inline void __de_mbus_reset(struct de_top_handle *hdl, bool reset)
{
	...
	if (hdl->private->dsc->support_mbus_reset) {
		reg_val = readl(reg_base);
		reg_val = DE_SET_BITS(shift, 1, reg_val, reset ? 0 : 1);
		writel(reg_val, reg_base);
	}
}
...
static int __de_mod_clk_enable(struct de_top_handle *hdl, enum de_clk_id clk_no, bool en)
{
	...
	reg_val = readl(reg_base);
	reg_val = DE_SET_BITS(shift, 1, reg_val, val);
...
static int de_top_set_clk_enable(struct de_top_handle *hdl, enum de_clk_id clk_no, u8 en)
{
	...
	if (en)
		count = ++hdl->private->mbus_ref_count;
	else
		count = --hdl->private->mbus_ref_count;
...
static int de_top_channel_clk_enable(struct de_top_handle *hdl, u32 chn_type_id, bool is_video)
{
	...
	if (hdl->private->dsc->support_channel_clk) {
		...
		reg_val = readl(reg_base);
		reg_val = DE_SET_BITS(shift, width, reg_val, 1);
		writel(reg_val, reg_base);
	}
	return 0;
}
...
static int de_top_set_de2tcon_mux(struct de_top_handle *hdl, u32 disp, u32 tcon)
{
	...
	reg_val = readl(reg_base);
	reg_val = DE_SET_BITS(shift, width, reg_val, tcon);
	...
	for (i = 0; i <= max_disp; i++) {
		if (i == disp)
			continue;
		shift = i << 2;
		if (DE_GET_BITS(shift, 4, reg_val) == tcon) {
			reg_val = DE_SET_BITS(shift, 4, reg_val, 0xf);
		}
	}
	writel(reg_val, reg_base);
	...
	u32 chn_type_id, bool is_video)
{
	...
	reg_val = DE_SET_BITS(chn_type_id << 1, 2, reg_val, disp);
	writel(reg_val, reg_base);
	return 0;
}
	...
	u32 chn_type_id, bool is_video)
{
	...
	reg_val = readl(reg_base);
	reg_val = DE_SET_BITS(chn_type_id << 2, 4, reg_val, disp);
	writel(reg_val, reg_base);
	return 0;
}
...
static int de_top_set_port2chn_mux(struct de_top_handle *hdl, u32 disp, u32 port, u32 chn_type_id, bool is_video)
{
	...
	reg_val = readl(reg_base);
	chn_type_id += is_video ? 0 : 8;
	reg_val = DE_SET_BITS(port << 2, 4, reg_val, chn_type_id);
	writel(reg_val, reg_base);
	return 0;
}
...
/*
	if (hdl->private->dsc->reserve_ctl_offset) {
		...
		reg_val = readl(reg_base);
		reg_val = DE_SET_BITS(8, 1, reg_val, 1);
		writel(reg_val, reg_base);
	}*/
	...
	reg_val = readl(reg_base);
	reg_val = DE_SET_BITS(shift, 1, reg_val, en ? 1 : 0);
	if (hdl->private->dsc->support_rcq_gate)
		reg_val = DE_SET_BITS(16, 1, reg_val, en ? 1 : 0);
	writel(reg_val, reg_base);
...
int de_top_selfwb_start(struct de_top_handle *hdl, bool en)
{
	u8 __iomem *de_base = hdl->cinfo.de_reg_base;
...
int de_top_wb_config(struct de_top_handle *hdl, const struct de_top_wb_cfg *cfg)
{
	...
	u8 __iomem *reg_base_ctl = de_base + hdl->private->dsc->rtwb_ctl_offset;
	u8 __iomem *reg_eink_mode = de_base + DE_RESERVE_CTRL_OFFSET;
	u32 reg_val;
	...
	if (!cfg->enable) {
		...
		if (cfg->mode >= SELF_GENERATED_TIMING) {
			reg_val = DE_SET_BITS(shift, 1, reg_val, 0);
			writel(reg_val, reg_base_ctl);
		}

		if (cfg->mode == SELF_GEN_TIMING_EINK) {
			reg_val = readl(reg_eink_mode);
			reg_val = DE_SET_BITS(16, 1, reg_val, 0x0);
			writel(reg_val, reg_eink_mode);
		}
	}
	...
	reg_val = readl(reg_base_ctl);
	if (cfg->mode >= SELF_GENERATED_TIMING) {
		reg_val = DE_SET_BITS(shift, 1, reg_val, 1);
		writel(reg_val, reg_base_ctl);
	}

	if (cfg->mode == SELF_GEN_TIMING_EINK) {
		reg_val = readl(reg_eink_mode);
		reg_val = DE_SET_BITS(16, 1, reg_val, 0x1);
		writel(reg_val, reg_eink_mode);
	}
	...
	if (cfg->pos == FROM_DISP)
		mux += 1;
	reg_val = DE_SET_BITS(shift, 3, reg_val, mux);
...
static s32 de_top_pingpang_buf_alloc(struct de_top_handle *hdl, unsigned int width, unsigned int height)
{
	...
	st->buff_size = ALIGN((width * height * 4 * 2), 4096);
...
static s32 de_top_set_offline_enable(struct de_top_handle *hdl, bool enable, enum de_offline_mode mode)
{
	...
	reg_val = readl(reg_base);
	reg_val = DE_SET_BITS(0, 1, reg_val, enable);
	reg_val = DE_SET_BITS(8, 1, reg_val, mode);
	reg_val = DE_SET_BITS(4, 1, reg_val, 1);
	writel(reg_val, reg_base);
	return 0;
}
...
int de_top_request_rcq_fifo_update(struct de_top_handle *hdl, u32 disp, unsigned long rcq_header_addr, unsigned int rcq_header_byte)
{
	...
	reg_val = readl(reg_base);
	reg_val = DE_SET_BITS(12, 1, reg_val, 1);
	writel(reg_val, reg_base);
