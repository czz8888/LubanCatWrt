...
struct sunxi_display_engine {
	...
	struct clk *mclk_bus;
	struct clk *mclk_ahb;
	struct reset_ctl *rst_bus_de;
	struct reset_ctl *rst_bus_de_sys;
	int irq_no;
	int crc_irq_no;
...
static int wb_rcq_head_switch(struct sunxi_de_out *hwde)
{
	...
	struct de_rcq_head *hd_end = hd ? hd + rcq_info->block_num : NULL;
...
void sunxi_de_reg_mem_deinit(struct sunxi_display_engine *de)
{
	if (de->match_data->update_mode == RCQ_MODE && de->reg.virt_addr) {
		dma_free_coherent((void *)(de->reg.virt_addr - de->reg.used_byte));
	}
}
		    ...
		    const struct sunxi_de_out_cfg *cfg)
{
	...
	struct reset_ctl *rsts[] = {engine->rst_bus_de, engine->rst_bus_de_sys};
	...
	sunxi_clk_enable(clks, ARRAY_SIZE(clks), rsts, ARRAY_SIZE(rsts));
...
void sunxi_de_disable(struct sunxi_de_out *hwde)
{
	...
	sunxi_clk_disable(clks, ARRAY_SIZE(clks), rsts, ARRAY_SIZE(rsts));
			      ...
			      struct sunxi_display_engine *engine)
{
	...
	engine->rst_bus_de = devm_reset_control_get(dev, "rst_bus_de");
	if (IS_ERR(engine->rst_bus_de)) {
		DRM_ERROR("Fail to get reset clk for rst_bus_de\n");
	}

	engine->rst_bus_de_sys = devm_reset_control_get(dev, "rst_bus_de_sys");
...
U_BOOT_DRIVER(de) = {
	...
	.priv_auto = sizeof(struct sunxi_display_engine),
};
