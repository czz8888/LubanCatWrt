...
{
	if (cstate->frontend_data) {
		return cstate->frontend_data->snr_para.en;
	} else
		return 0;
}

static void de_frontend_calculate_update_local_param_work(struct de_frontend_private *priv)
{
	if (priv->dci)
...
void de_frontend_process_late(struct de_frontend_handle *hdl)
{
	...
	if (hdl->private->has_frontend_work)
		de_frontend_calculate_update_local_param_work(hdl->private);
}
...
s32 de_frontend_apply_snr(struct de_frontend_handle *hdl, struct display_channel_state *state)
{
	...
	if (hdl->private->snr) {
		if (state->frontend_data) {
			de_snr_set_para(hdl->private->snr, state, &state->frontend_data->snr_para);
		} else {
...
static s32 de_frontend_set_size(struct de_frontend_handle *hdl, struct de_frontend_inner_info *info)
{
	...
	DRM_INFO("demo_enable=%d, size=%d,%d, size2=%d,%d\n", demo_enable,
		 ...
		 struct de_frontend_apply_cfg *frontend_cfg)
{
	...
	/*check bypass or not*/
	if (cstate->frontend_data) {
		frontend_data = cstate->frontend_data;

		new_inner_info->demo_window_en = frontend_data->demo_en;

		if (frontend_data->snr_para.bypass)
			new_inner_info->bypass |= BYPASS_SNR_MASK;
		if (frontend_data->dci_para.bypass)
			new_inner_info->bypass |= BYPASS_DCI_MASK;
		if (frontend_data->cdc_para.bypass)
			new_inner_info->bypass |= BYPASS_CDC_MASK;
		if (frontend_data->csc1_para.bypass)
			new_inner_info->bypass |= BYPASS_CCSC_MASK;
//TODO
//		if (frontend_data->csc2_para.bypass)
//			new_inner_info->bypass |= BYPASS_CCSC_MASK;
		if (frontend_data->fcm_para.bypass)
			new_inner_info->bypass |= BYPASS_FCM_MASK;
		if (frontend_data->sharp_para.bypass)
...
s32 de_frontend_apply_lut(struct de_frontend_handle *hdl, struct display_channel_state *cstate)
{
	...
	if (cstate->frontend_data) {
		...
		if (hdl->private->fcm && frontend_data->fcm_para.lut_need_update)
			de_fcm_apply_lut(hdl->private->fcm, &frontend_data->fcm_para.fcm_lut_data, 1);
	}
