...
#ifdef DRM_FORMAT_ARGB8565
	case DRM_FORMAT_ARGB8565:
		return DE_FORMAT_ARGB_8565;
#endif
#ifdef DRM_FORMAT_ABGR8565
	case DRM_FORMAT_ABGR8565:
		return DE_FORMAT_ABGR_8565;
#endif
#ifdef DRM_FORMAT_RGBA5658
	case DRM_FORMAT_RGBA5658:
		return DE_FORMAT_RGBA_5658;
#endif
#ifdef DRM_FORMAT_BGRA5658
	case DRM_FORMAT_BGRA5658:
		return DE_FORMAT_BGRA_5658;
#endif
	}
...
{
	unsigned int en_cnt = chn_info->layer_en_cnt;
	...
	de_scaler_calc_ovl_scale_para(en_cnt, ypara, cpara,
...
static u32 de_rtmx_chn_fix_size_for_ability(const struct de_chn_info *chn_info, unsigned int de_ouput_height, unsigned int device_fps, unsigned long de_freq, u32 *height_out)
{
	...
	*/
	tmp = de_ouput_height * device_fps *
		...
		? chn_info->ovl_win.width : chn_info->scn_win.width);
	if (tmp != 0) {
		update_speed_ability = (u64)de_freq * 80;
	...
	required_speed_ability = (chn_info->scn_win.height == 0) ? 0 :
		((u64)chn_info->ovl_out_win.height * 100 / chn_info->scn_win.height);
...
bool channel_format_mod_supported(struct de_channel_handle *hdl, uint32_t format, uint64_t modifier)
{
	if (modifier == DRM_FORMAT_MOD_LINEAR)
		return true;
	if (hdl->private->afbd)
		return de_afbc_format_mod_supported(hdl->private->afbd, format, modifier);
	return false;
}
		    ...
		    bool rgb_out)
{
	...
	if (state->base.fb == NULL) {
		if (hdl->private->afbd)
			de_afbd_apply_lay(hdl->private->afbd, state, &afbd_cfg, &enable);
	...
	ovl_cfg.layer_en_cnt = enable ? 0 : hdl->private->info.layer_en_cnt;
				...
				struct drm_framebuffer **fb, unsigned int fb_cnt)
{
	...
	drm_printf_indent(p, indent, "fb_id    : %20d |%20d |%20d |%20d\n",
		    fb[0] ? fb[0]->fb_id : -1, fb[1] ? fb[1]->fb_id : -1,
		    fb[2] ? fb[2]->fb_id : -1, fb[3] ? fb[3]->fb_id : -1);
	...
	drm_printf_indent(p, indent, "p0 paddr : 0x%18llx |0x%18llx |0x%18llx |0x%18llx\n",
		    fb[0] ? (unsigned long long)(fb[0]->dma_addr) : 0, fb[1] ? (unsigned long long)(fb[1]->dma_addr) : 0,
		    fb[2] ? (unsigned long long)(fb[2]->dma_addr) : 0, fb[3] ? (unsigned long long)(fb[3]->dma_addr) : 0);
	...
	for (i = 0; i < cnt; i++) {
		if (i == 0) {
			drm_printf(p, "\t\t   %3d    |"" %4d  |"" %4dx%4d+%4d+%4d | %4dx%4d+%4d+%4d |  %8s  |   %3d  | 0x%08x\n",
					i,  s->fb ? s->fb->fb_id : 0, s->src_w >> 16, s->src_h >> 16,
			...
			drm_printf(p, "\t\t   %3d    |"" %4d  |"" %4dx%4d+%4d+%4d | %4dx%4d+%4d+%4d |  %8s  |   %3d  | 0x%08x\n",
					i, state->fb[i - 1] ? state->fb[i - 1]->fb_id : 0, state->src_w[i - 1] >> 16, state->src_h[i - 1] >> 16,
...
struct de_channel_handle *de_channel_create(struct module_create_info *cinfo)
{
	...
	if (hdl->private->afbd) {
		...
		hdl->format_modifiers = hdl->private->afbd->format_modifiers;
	} else {
		hdl->format_modifiers = format_modifiers_common;
	}
