					...
					int format, struct drm_framebuffer *fb, unsigned int layer_id)
{
	...
	if (format <= DE_FORMAT_BGRX_8888) {
		...
		((format >= DE_FORMAT_ARGB_8565) && (format <= DE_FORMAT_BGRA_5658))) {
		bpp[0] = 24;
	...
	addr_tmp = (u64)(fb->dma_addr) + fb->offsets[1];
	addr[1] = addr_tmp
		+ pitch[1] * y1 + (x1 * bpp[1] >> 3);

	addr_tmp = (u64)(fb->dma_addr) + fb->offsets[2];
	addr[2] = addr_tmp
	...
	u32 *fmt, u32 *vi_ui_sel)
{
	...
	case DE_FORMAT_YUV444_I_VUYA:
		...
		*fmt = 0x3;
		break;
	case DE_FORMAT_ARGB_8565:
		*vi_ui_sel = 0x1;
		*fmt = 0x14;
		break;
	case DE_FORMAT_ABGR_8565:
		*vi_ui_sel = 0x1;
		*fmt = 0x15;
		break;
	case DE_FORMAT_RGBA_5658:
		*vi_ui_sel = 0x1;
		*fmt = 0x16;
		break;
	case DE_FORMAT_BGRA_5658:
		*vi_ui_sel = 0x1;
		*fmt = 0x17;
		break;
	default:
