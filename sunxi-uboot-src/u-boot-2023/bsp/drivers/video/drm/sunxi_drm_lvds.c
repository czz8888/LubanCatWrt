...
#if IS_ENABLED(CONFIG_MACH_SUN8IW22) || IS_ENABLED(CONFIG_MACH_SUN55IW6)
...
{
	int ret = 0;
	struct reset_ctl *rsts[] = {lvds->rst_bus};

	ret = sunxi_clk_enable(NULL, 0, rsts, ARRAY_SIZE(rsts));
...
static int sunxi_lvds_clk_config_disable(struct sunxi_drm_lvds *lvds)
{
	...
	ret = sunxi_clk_disable(NULL, 0, rsts, ARRAY_SIZE(rsts));
					  ...
					  struct display_state *state)
{
	...
	ret = pinctrl_select_state(lvds->dev, "active");
	if (ret < 0) {
...
s32 sunxi_lvds_parse_dt(struct udevice *dev)
{
	...
	ret = ofnode_read_u32(of_node, "lvds-phy-c", &value);
	if (!ret)
		lvds_para->lvds_reg_c = value;
	else
		lvds_para->lvds_reg_c = 4;

	lvds->rst_bus = devm_reset_control_get(dev, "rst_bus_lvds");
	if (IS_ERR(lvds->rst_bus)) {
		DRM_ERROR("fail to get reset rst_bus_mipi_lvds\n");
	}
...
U_BOOT_DRIVER(sunxi_drm_lvds) = {
	...
	.priv_auto = sizeof(struct sunxi_drm_lvds),
};
