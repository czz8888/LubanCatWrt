...
{
	struct device_node *ep_node, *panel_node, *bus_node;
	ofnode panel_ofnode, bus_ofnode, port;
	struct udevice *panel_dev, *bus_dev;
	int ret = 0;
	...
	*panel = NULL;
	...
	bus_ofnode = dev_read_subnode(dev, "bus");
	if (ofnode_valid(bus_ofnode) && ofnode_is_enabled(bus_ofnode)) {
		ret = uclass_get_device_by_ofnode(UCLASS_MISC, bus_ofnode,
						  &bus_dev);
		if (!ret)
			goto found;
	}
	...
	bus_node = sunxi_of_graph_get_port_parent(port);
	if (!bus_node)
		return -ENODEV;
	...
	ret = uclass_get_device_by_ofnode(UCLASS_MISC, np_to_ofnode(bus_node), &bus_dev);
	if (!ret)
		goto found;
...
struct sunxi_drm_device *sunxi_drm_device_get(void)
{
	...
	if (uclass_get_device_by_driver(UCLASS_VIDEO,
					DM_DRIVER_GET(sunxi_display), &dev)) {
		return NULL;
	}
...
static int sunxi_drm_drv_probe(struct udevice *dev)
{
	...
	const char *name;
	int ret = -1, phandle[2] = {-1, -1}, phandle_count = 0, i = 0;
	...
	ofnode_for_each_subnode(node, route_node) {
		...
		phandles = ofnode_read_prop(node, "endpoints", &phandle_count);
		phandle_count /= sizeof(fdt32_t);
		for (i = 0; i < phandle_count; ++i) {
			phandle[i] = fdt32_to_cpu(phandles[i]);
			if (phandle[i] < 0) {
...
#if IS_ENABLED(CONFIG_SUNXI_TINYJPEG_DECODER)
int load_jpeg_logo(struct display_state *state, char *jpeg_name)
{
	int ret = 0;
	unsigned char *bmp_buff = NULL, *body = NULL;
	unsigned int bmp_size, width, height;
	struct jdec_private *jdec = NULL;
	struct bmp_header *header = NULL;
	struct file_info_t *logo = NULL;

	if (state->logo) {
		if (state->logo->unload_file)
			state->logo->unload_file(state->logo);
		state->logo = NULL;
	}

	logo = load_file(jpeg_name, "boot-resource");
	if (!logo)
		logo = load_file(jpeg_name, "bootloader");

	if (!logo) {
		DRM_ERROR("can not load file %s.\n", jpeg_name);
		goto loader_failed;
	}

	jdec = tinyjpeg_init();
	if (IS_ERR_OR_NULL(jdec)) {
		DRM_ERROR("tinyjpeg init failed!\n");
		goto free_file;
	}

	ret = tinyjpeg_parse_header(jdec, logo->file_addr, logo->file_size);
	if (ret < 0) {
		DRM_ERROR("tinyjpeg parse header failed!\n");
		goto free_jpeg;
	}

	tinyjpeg_get_size(jdec, &width, &height);

	bmp_size = sizeof(struct bmp_header) + width * height * 4;
	bmp_buff = memalign(4096, bmp_size);
	if (IS_ERR_OR_NULL(bmp_buff)) {
		DRM_ERROR("malloc bmp buffer failed\n");
		goto free_jpeg;
	}

	header = (struct bmp_header *)bmp_buff;
	...
	body = bmp_buff + sizeof(struct bmp_header);
	tinyjpeg_set_components(jdec, &body, 1);

	ret = tinyjpeg_decode(jdec, TINYJPEG_FMT_BGRA32);
	if (ret < 0) {
		DRM_ERROR("tinyjpeg decode failed!\n");
		goto free_bmp;
	}

	if (logo->unload_file)
		logo->unload_file(logo);

	state->logo = (struct file_info_t *)malloc(sizeof(struct file_info_t));
	if (IS_ERR_OR_NULL(state->logo)) {
		DRM_ERROR("malloc logo buffer failed!\n");
		goto free_bmp;
	}

	memset(state->logo, 0, sizeof(struct file_info_t));
	state->logo->file_addr = bmp_buff;
	state->logo->file_size = bmp_size;
	return 0;
...
int load_jpeg_logo(struct display_state *state, char *jpeg_name)
{
	DRM_ERROR("unsupport jpeg decoder\n");
	return -1;
}
#endif /* SUNXI_TINYJPEG_DECODER */
...
static int display_logo(struct display_state *state)
{
	...
	bmp_display((ulong)state->logo->file_addr, BMP_ALIGN_CENTER, BMP_ALIGN_CENTER);
...
int sunxi_show_logo(void)
{
	...
	sunxi_drm_for_each_display(s, drm) {
		...
		id = s->crtc_state.crtc->hw_id;
		p = strrchr(s->ulogo_name, '.');

		if (p && ((strcmp(p, ".jpg") == 0) || (strcmp(p, ".jpeg") == 0)))
			ret = load_jpeg_logo(s, s->ulogo_name);
		else
			ret = load_bmp_logo(s, s->ulogo_name);
		if (ret) {
			DRM_ERROR("disp %d load logo %s failed!\n", id, s->ulogo_name);
			continue;
		}
...
static int do_sunxi_logo_show(struct cmd_tbl *cmdtp, int flag, int argc,
			char *const argv[])
{
	if (argc != 1)
...
static int do_sunxi_show_bmp(struct cmd_tbl *cmdtp, int flag, int argc,
				char *const argv[])
{
	if (argc != 2)
		return CMD_RET_USAGE;
...
static int do_sunxi_sunxi_drm(struct cmd_tbl *cmdtp, int flag, int argc,
				char *const argv[])
{
