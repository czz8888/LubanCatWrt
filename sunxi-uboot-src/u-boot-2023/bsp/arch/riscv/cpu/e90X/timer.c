...
{
	u64 t1, t2;
	int hosc_freq = aw_get_hosc_freq();
...
/* get the current time(ms), freq = hosc_freq Hz*/
int runtime_tick(void)
{
	...
	cnt = read_timer();
	return lldiv(cnt, (hosc_freq * 1000));
}
...
void add_timer(struct timer_list *timer)
{
	...
	__maybe_unused int hosc_freq = aw_get_hosc_freq();
...
#ifndef FPGA_PLATFORM
	timer_tcontrol->inter = timer->expires * ((hosc_freq * 1000) / 32);
#else
