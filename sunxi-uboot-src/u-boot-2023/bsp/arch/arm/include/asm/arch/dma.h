...
#define DMAC_CFG_SRC_TYPE_NAND (5)
	...
	defined(CONFIG_MACH_SUN55IW6) || defined(CONFIG_MACH_SUN8IW22)
...
typedef struct {
	...
	defined(CONFIG_MACH_SUN20IW5) || defined(CONFIG_MACH_SUN8IW22)
typedef struct {
...
#if defined(CONFIG_MACH_SUN60IW2) || defined(CONFIG_MACH_SUN55IW6) || defined(CONFIG_MACH_SUN8IW22)
...
#if defined(CONFIG_MACH_SUN55IW3) || defined(CONFIG_MACH_SUN8IW21)
...
struct sunxi_dma_dev;

struct sunxi_dma_config {
	u32 channum_per_reg;
	void (*irq_enable)(struct sunxi_dma_dev *sdev, u32 chan_num, u32 irq_val);
	u32 (*read_irq_enable)(struct sunxi_dma_dev *sdev, u32 chan_num);
	u32 (*get_irq_status)(struct sunxi_dma_dev *sdev, u32 chan_num);
	void (*clear_irq_status)(struct sunxi_dma_dev *sdev, u32 chan_num, u32 status);
	void (*set_auto_gate)(struct sunxi_dma_dev *sdev, u32 reg_val);
};

struct sunxi_dma_dev {
	void __iomem    *base;
	struct clk clk_bus, clk_mbus;
	struct reset_ctl reset;
	struct sunxi_dma_config *cfg;
};

#define DMA_IRQ_EN(x)           ((x) * 0x04)
#define DMA_IRQ_STAT(x)         ((x) * 0x04 + 0x10)
#define DMA_IRQ_CHAN_NR         8
#define DMA_AUTO_GATE		0x28
