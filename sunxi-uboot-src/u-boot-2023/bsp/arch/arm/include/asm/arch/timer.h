...
struct sunxi_timer {
	volatile u32 ctl;  /* timer control */
	volatile u32 inter;  /* timer low interval value*/
	volatile u32 val;  /* timer low current value value*/
	volatile u32 inter2;  /* timer high interval value*/
	volatile u32 val2;  /* timer high current value value*/
	volatile u32 cvrs;  /* timer current value read select*/
};
#else
struct sunxi_timer {
	volatile u32 ctl;
...
#elif defined(CONFIG_MACH_SUN55IW6) || defined(CONFIG_MACH_SUN8IW22)
struct sunxi_timer_reg {
	volatile u32 tirqen; /* 0x00 */
	volatile u32 tirqsta; /* 0x04 */
	volatile u32 tsecctl; /* 0x08 */
	volatile u32 tver; /* 0x0C */
	uint res1[4];
	struct sunxi_timer timer[7]; /* 0x20 We have 7 timers */
};
#else
struct sunxi_timer_reg {
	u32 tirqen;		/* 0x00 */
