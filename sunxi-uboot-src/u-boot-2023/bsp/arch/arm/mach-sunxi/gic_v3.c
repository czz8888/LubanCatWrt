 ...
 * Tom Cubie <huanrgongcun@allwinnertech.com>
 *
...
__weak void clock_set_gic(void)
{
	pr_info("__weak %s...%d\n", __func__, __LINE__);
	return ;
}
...
{
	printf("default_isr():  called from IRQ %p\n", data);
	while (1)
		;
}

static __inline void sr32(ulong addr, u32 start_bit, u32 num_bits, u32 value)
{
...
void do_irq(struct pt_regs *pt_regs)
{
	...
	if ((idnum == 1022) || (idnum == 1023)) {
		...
		/* FIXME: The log output makes timer irq lost.
		 * timer irq handler work right even if the condition met.
		 */
		debug("spurious irq !!, idnum: %d\n", idnum);
#else
		printf("spurious irq !!\n");
#endif
		return;
...
{
	u32 temp = 0;
	asm volatile("mrs  %0, s3_0_c4_c6_0" : "=r"(temp));
	temp &= ~(0xFF << 0);
	temp |= prio_mask;
	asm volatile("msr s3_0_c4_c6_0, %0" ::"r"(temp));
}

static u32 get_int_ack_num(void)
{
	u32 irq_num = 0;
	asm volatile("mrs  %0, s3_0_c12_c12_0" : "=r"(irq_num));
	return irq_num;
}

static void gic_end_of_int(u32 irq_num)
{
	asm volatile("msr s3_0_c12_c12_1, %0"::"r"(irq_num));
}

static void gic_deact_int(u32 irq_num)
{
	asm volatile("msr s3_0_c12_c11_1, %0"::"r"(irq_num));
...
{
	u32 value = 1;
	asm volatile("msr s3_0_c12_c12_5, %0"::"r"(value));
}

static void gic_group_enable(void)
{
	u32 value = 1;
	asm volatile("msr s3_0_c12_c12_7, %0"::"r"(value));
