...
#include <vxworks.h>
#include <asm/cache.h>
#include <sunxi_arisc.h>
#include <sunxi_hyper.h>
...
static void announce_and_cleanup(int fake)
{
	...
	sunxi_info(NULL, "Starting kernel ...%s\n\n", fake ?
...
static void boot_jump_linux(struct bootm_headers *images, int flag)
{
	...
	announce_and_cleanup(fake);
		...
		sunxi_smc_call_atf(ARM_SVC_ARISC_STARTUP, (ulong)images->ft_addr, 0, 0, 0);
#endif
#endif
	if (!fake) {
	...
	if (!fake) {
		...
		/* load and de-assert cpus before kernel*/
		sunxi_smc_call_atf(ARM_SVC_ARISC_STARTUP, (ulong)r2, 0, 0, 0);
#endif
#ifdef CONFIG_SUNXI_ARM64
			if (sunxi_probe_secure_monitor() &&
			    (sunxi_get_force_32bit_os() == 0)) {
				if (kylo_hyper_entry())
					arm_svc_run_hyper((ulong)kylo_hyper_entry(),
							(ulong)r2, 1);
				else
					arm_svc_run_os((ulong)kernel_entry,
							(ulong)r2, 1);
			} else
#endif
		   ...
		   struct bootm_headers *images)
{
	...
	if (!kylo_load_hyper_img()) {  /* only have hyper partition, will run here */
		u64 initrd_start, initrd_size;
		if (images->initrd_start && images->initrd_end) {
			initrd_start = (u64)images->initrd_start;
			initrd_size = (u64)(images->initrd_end - images->initrd_start);
		} else if (images->rd_start && images->rd_end) {
			initrd_start = (u64)images->rd_start;
			initrd_size = (u64)(images->rd_end - images->rd_start);
		}

		kylo_dtb_update(working_fdt, DOM1_RAMDISK_NODE,
				initrd_start, initrd_size);

		kylo_dtb_update(working_fdt, DOM1_KERNEL_NODE,
				(u64)images->ep, (u64)images->os.image_len);

		if (kylo_load_os_img())
			printf("load dom2 boot image fail\n");
	}
